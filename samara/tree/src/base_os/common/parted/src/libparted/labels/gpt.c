	...
	if (!asked_already && last_usable < last_usable_if_grown) {
		...
		q = ped_exception_throw(PED_EXCEPTION_WARNING,
		                        ...
		                        _("Not all of the space available to %s appears "
		                          "to be used, you can fix the GPT to use all "
		                          "of the space (an extra %llu blocks) or "
		  ...
		  GuidPartitionTableHeader_t** gpt_p)
{
	...
	gpt->HeaderSize = PED_CPU_TO_LE32 (pth_get_size_static (disk->dev)); /* per 1.00 spec */
	gpt->HeaderCRC32 = 0;
	gpt->Reserved1 = 0;
...
gpt_alloc_metadata (PedDisk * disk)
{
	...
	gptlength = ped_div_round_up (sizeof (GuidPartitionTableHeader_t), disk->dev->sector_size);
