...
do_mkpart (PedDevice** dev)
{
        ...
        if (!ped_disk_add_partition (disk, part, final_constraint)) {
                ...
                if (ped_disk_add_partition (disk, part,
                        ...
                        if (!start_usr || !end_usr || !start_sol || !end_sol ||
                            strcmp(start_usr, start_sol) ||
                            strcmp(end_usr, end_sol)) {
...
_init_commands ()
{
        command_register (commands, command_create (
                str_list_create_unique ("busy", _("busy"), NULL),
                do_checkbusy,
                str_list_create (
_("busy                                     check if this disk is busy"),
NULL),
