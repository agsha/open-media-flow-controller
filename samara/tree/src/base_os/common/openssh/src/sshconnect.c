		...
		if (options.strict_host_key_checking == 1) {
			...
			if (!confirm(msg))
				goto fail;
                        ...
                        /* Automatically accept an unknown host in 'accept'
                         * mode, don't prompt either */
			char msg1[1024], msg2[1024];
			...
			logit("The authenticity of host '%.200s (%s)' can't "
                              "be established%s\n"
                              "%s key fingerprint is %s.\n%s"
                              "Adding host to the list of known hosts",
                              host, ip, msg1, type, fp, msg2);
			xfree(fp);
		}
		if (options.check_host_ip && ip_status == HOST_NEW) {
		...
		if (options.strict_host_key_checking) {
			error("%s host key for %.200s has changed and you have "
			    "requested strict checking.", type, host);
                        ...
                        syslog(LOG_WARNING, "Remote host identification "
                               "for host %s,%s with key fingerprint %s"
                               " has changed, can not continue with strict"
                               " checking enabled", host, ip, fp);
			...
			goto fail;
                    ...
                    syslog(LOG_WARNING, "Remote host identification for "
                           "host %s,%s with key fingerprint %s has changed",
                           host, ip, fp);
                    closelog();
                    xfree(fp);
                }
