...
process_open(void)
{
	...
	TRACE("open id %u name %s flags %d mode 0%o", id, name, pflags, mode);
        ...
        if ((flags & O_RDWR) == O_RDWR) {
            ...
            /*
             * This is annoying.  The 'open' man page says:
             * "The parameter flags must include one of the following
             * access modes: O_RDONLY, O_WRONLY, or O_RDWR."
             * Yet we are known to be invoked this way sometimes, such
             * as when the sftp client issues a "get" command!
             *
             * To honor our rules, we want to be able to distinguish
             * between reads and writes, so if we can't figure it out
             * when the file is being opened, we have to allow it
             * here, and then check at process_read() or
             * process_write().  HOWEVER, we have gotten around having
             * to do this for now, by requiring that ALL rules apply to
             * both reads and writes (i.e. they cannot distinguish), and
             * treating undeclared file opens as O_RDWR.
             */
            opers = tfot_read | tfot_write;
        }

        if (!tms_path_check_ok(name, opers)) {
...
process_read(void)
{
	...
	TRACE("read id %u handle %d off %" PRIu64 " len %d", id, handle,
        ...
        /* XXX/EMT: could enforce read limitations, ideally once per fd */
...
process_write(void)
{
	...
	TRACE("write id %u handle %d off %" PRIu64 " len %d", id, handle,
        ...
        /* XXX/EMT: could enforce write limitations, ideally once per fd */
...
#ifdef PROD_FEATURE_RESTRICT_CMDS
#ifndef ssh_restrict_rules_NONE
        if (!tms_path_check_ok(name, tfot_read)) {
            status = SSH2_FX_PERMISSION_DENIED;
            goto done;
        }
#endif /* ssh_restrict_rules_NONE */
...
#ifdef PROD_FEATURE_RESTRICT_CMDS
#ifndef ssh_restrict_rules_NONE
        if (!tms_path_check_ok(name, tfot_write)) {
            status = SSH2_FX_PERMISSION_DENIED;
            goto done;
        }
#endif /* ssh_restrict_rules_NONE */
...
#ifdef PROD_FEATURE_RESTRICT_CMDS
#ifndef ssh_restrict_rules_NONE
        if (!tms_path_check_ok(path, tfot_read)) {
            status = SSH2_FX_PERMISSION_DENIED;
            goto done;
        }
#endif /* ssh_restrict_rules_NONE */
...
#ifdef PROD_FEATURE_RESTRICT_CMDS
#ifndef ssh_restrict_rules_NONE
        if (!tms_path_check_ok(oldpath, tfot_write) ||
            !tms_path_check_ok(newpath, tfot_write)) {
            status = SSH2_FX_PERMISSION_DENIED;
            goto done;
        }
#endif /* ssh_restrict_rules_NONE */
