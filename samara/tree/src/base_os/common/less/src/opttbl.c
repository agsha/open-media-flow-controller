...
/*
 * This used to be for the '-A' option to support paging in the CLI.
 * It is no longer necessary, but we continue to accept the option
 * (and silently ignore it) so as not to break customers who 
 * developed any usage of 'less' in the past 8 years.  The structure
 * we use to add the option wants a place to put the answer, so this is
 * the dummy variable we use for that.
 */
public int dont_skip_to_end;
...
static struct optname dont_skip_to_end_optname = { "dont-skip-to-end", NULL };
...
static struct loption option[] =
{
	...
	/*
	 * This option is ignored, but left here for backward
	 * compatibility.  See comments above dont_skip_to_end.
	 */
	{ 'A', &dont_skip_to_end_optname,
		BOOL, OPT_OFF, &dont_skip_to_end, NULL,
		{
			"Skip to end of screen (IGNORED)",
			"Don't skip to end of screen (IGNORED)",
			NULL
		}
	},
