                                  ...
                                  int type)
{
    ...
    if (SSL_X509_getBC(cert, &is_ca, &pathlen)) {
        if (is_ca) {
            /*
             * XXX/EMT: this used to be at APLOG_WARNING, but was
             * lowered (for now) because we can't get rid of the
             * warning without automatically regenerating our
             * self-signed cert, which would be disruptive to users
             * (their browsers would all complain again, and they'd
             * have to add a new exception).
             *
             * This might be more appropriate at APLOG_INFO, but our
             * config file (generated by mgmtd) currently hardwires our
             * own log level at NOTICE, so this would never get through.
             * Apache is somewhat chatty at INFO level (it logs about
             * 15 extra messages at startup, though no additional ones
             * that I saw in subsequent normal operation).  So just
             * to avoid rocking the boat, we put this at NOTICE for now.
             */
