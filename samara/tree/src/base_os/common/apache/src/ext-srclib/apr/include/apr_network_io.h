...
/*
 * TMS: for apr_sockaddr_info_get() / find_addresses() / call_resolver(),
 * don't use AI_ADDRCONFIG in our call to getaddrinfo().
 *
 * (For bug 14713, we use this flag because we have already carefully
 * written httpd.conf to have the addrs we want, and we don't want to
 * be second-guessed.  Starting under EL6, AI_ADDRCONFIG will not 
 * consider the loopback address to be relevant for a given family,
 * but we want it to be.)
 */
#define APR_GAI_NO_ADDRCONFIG (1 << 30) 
    
/*
 * TMS: for apr_sockaddr_info_get() / find_addresses() / call_resolver(),
 * use AI_NUMERICHOST in our call to getaddrinfo().
 *
 * (For bug 14713, we use this flag because we only ever write IP
 * addresses into httpd.conf, so it can't help us to have them possibly
 * used for a name lookup.)
 */
#define APR_GAI_NUMERICHOST   (1 << 29)
