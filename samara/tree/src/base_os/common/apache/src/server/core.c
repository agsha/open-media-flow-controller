...
static void set_banner(apr_pool_t *pconf)
{
    if (ap_server_tokens == SrvTk_PRODUCT_ONLY) {
        ...
        /*
         * XXX/EMT: the "15" is suspicious here given that
         * AP_SERVER_PATCHLEVEL_NUMBER is 15.  But AP_SERVER_BASEVERSION
         * does include our extra string, so it seems we don't have to
         * meddle here...
         */
        ap_add_version_component(pconf, AP_SERVER_BASEVERSION " (CentOS) (Release 15.el6.1)");
    }
    else {        
        ap_add_version_component(pconf, AP_SERVER_BASEVERSION " (CentOS)");
    }
        ...
        if (getrlimit(RLIMIT_CORE, &lim) == 0 && lim.rlim_cur == 0) {
            ...
            if (setrlimit(RLIMIT_CORE, &lim) == 0) {
                ap_log_error(APLOG_MARK, APLOG_NOTICE, 0, NULL,
                             "core dump file size limit raised to %llu bytes",
