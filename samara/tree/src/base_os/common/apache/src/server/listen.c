                                  ...
                                  apr_port_t port, const char* proto)
{
    ...
    /*
     * TMS: these two flags are passed to avoid bug 14713.
     */
    if ((status = apr_sockaddr_info_get(&sa, addr, APR_UNSPEC, port, 
                                        APR_GAI_NO_ADDRCONFIG |
                                        APR_GAI_NUMERICHOST,
                                        process->pool))
        != APR_SUCCESS) {
...
static int open_listeners(apr_pool_t *pool)
{
    ...
    /*
     * Apache used to do this *after* opening the new listeners 
     * (the loop which is now right after us).  Not sure why, since
     * any listeners it wanted to reuse were already moved out of 
     * this list.  The closing was moved up here as part of the 
     * fix for bug 13834: due to the semantics of SO_REUSEADDR,
     * in certain cases the server would trip over its own old
     * listeners.
     */
    ...
    /*
     * If we succeeded in binding to all our sockets, reset the
     * expected exit code override which we might have set after
     * receiving a SIGUSR1.  We don't want the override persisting and
     * taking effect later during an exit for an unrelated reason.
     */
    ap_exit_code_override = -1;
