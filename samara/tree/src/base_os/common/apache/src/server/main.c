                                     ...
                                     int process_exit_value)
{
    ...
    apr_terminate();
    if (ap_exit_code_override >= 0) {
        exit(ap_exit_code_override);
    }
    else {
...
static int abort_on_oom(int retcode)
{
    ...
    do {
        ret = write(STDERR_FILENO, OOM_MESSAGE, strlen(OOM_MESSAGE));
...
int main(int argc, const char * const argv[])
{
    ...
    /* --------------------------------------------------------------------- */
    /* TMS change: block AP_SIG_GRACEFUL until we register a handler
     * for it, lest it cause us to exit (see bug 14396).
     *
     * NOTE: we assume here we are using the 'prefork' MPM, since we
     * are not editing any of the others to unblock this signal.
     */
    int sigerr = 0;
    sigset_t sigset;
    sigemptyset(&sigset);
    sigerr = sigprocmask(SIG_BLOCK, NULL, &sigset);
    if (sigerr != 0) {
        exit(1);
    }
    ...
    for (;;) {
        ...
        if (ap_run_open_logs(pconf, plog, ptemp, server_conf) != OK) {
            /*
             * ap_run_open_logs() is a misleading name, because this
             * function doesn't just open logs.  It also calls
             * ap_setup_listeners(), which is what tries to bind to
             * our listening socket(s).  So if we have a problem
             * (e.g. bug 13834), we don't want to log something
             * misleading like "Unable to open logs", which Apache
             * had here before.
             */
            ap_log_error(APLOG_MARK, APLOG_STARTUP |APLOG_ERR,
                         0, NULL, "Initialization (open logs, setup "
                         "listeners, etc.) failed");
            destroy_and_exit_process(process, 1);
        }
