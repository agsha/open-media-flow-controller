...
/* 
 * NOTE: This file had been modified by Tall Maple Systems, Inc. 
 *
 */
...
#ifdef HAVE_SYSLOG
    /*
     * Make sure we call openlog() if we haven't already.  If we let it
     * auto-init, we won't get LOG_PID which can sometimes be useful for
     * distinguishing log messages of children from those of the parent.
     *
     * The flags here, and LOG_LOCAL7, are copied from the openlog()
     * calls above that are part of Apache.  (And LOG_LOCAL7 luckily
     * happens to be what we want it to use anyway, per
     * lc_log_facility_name_map)
     */
    if (!ap_openlog_called) {
        openlog(ap_server_argv0, LOG_NDELAY|LOG_CONS|LOG_PID, LOG_LOCAL7);
        ap_openlog_called = 1;
    }
