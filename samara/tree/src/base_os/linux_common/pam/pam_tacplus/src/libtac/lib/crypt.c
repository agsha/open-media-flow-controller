...
u_char *_tac_md5_pad(char *secret, int len, HDR *hdr)  {
 int n, i, bufsize;
 ...
 bufsize=sizeof(hdr->session_id) + strlen(secret) + sizeof(hdr->version)
 ...
 /* MD5_1 = MD5{session_id, secret, version, seq_no}
  ...
  bcopy(secret, buf+bp, strlen(secret));
  bp+=strlen(secret);
...
/* Perform encryption/decryption on buffer. This means simply XORing
   ...
   pad. */
void _tac_crypt(char *secret, u_char *buf, HDR *th, int length) {
 int i;
 u_char *pad;
 
 /* null operation if no encryption requested */
 if ((th->encryption == TAC_PLUS_ENCRYPTED) && secret){
 
  pad=_tac_md5_pad(secret, length, th);
