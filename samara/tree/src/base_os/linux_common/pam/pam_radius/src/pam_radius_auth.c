/*
 * $Id: pam_radius_auth.c,v 1.20 2013/01/26 03:39:26 et Exp $
 * pam_radius_auth
...
/* 
 * NOTE: This file has been modified by Tall Maple Systems (TMS), Inc. 
 *
 * The changes are:
 *
 *  - Parsing of the configuration file has been modified to handle host
 *    secrets containing white space.  This is handle by contract between the TMS
 *    software that generates configuration strings containing escape sequences
 *    for these characters and a call that restores the original via an
 *    unescape function here.  See escape_utils.h for unescape function details.
 */

/* TMS config file string unescape definitions */
#define CFG_ESC_CHR '_'
...
static void _pam_log_simple(int err, CONST char *format, ...)
    __attribute__ ((format (printf, 2, 3)));

#define DPRINT if (ctrl & PAM_DEBUG_ARG) _pam_log_simple
...
/* argument parsing */
static int _pam_parse(pam_handle_t *pamh, int argc, CONST char **argv, radius_conf_t *conf)
{
  int ctrl=0;
  ...
  for (ctrl=0; argc-- > 0; ++argv) {
    ...
    if (!strncmp(*argv,"conf=",5)) {
      ...
      if (conf->client_id) {
	_pam_log(pamh, LOG_WARNING, "ignoring duplicate '%s'", *argv);
      } else {
 ...
 */
static int
verify_packet(const char *secret, AUTH_HDR *response, AUTH_HDR *request)
{
  MD5_CTX my_md5;
...
add_attribute(AUTH_HDR *request, unsigned char type, CONST unsigned char *data, int length)
{
  ...
  len = ntohs(request->length) + p->length;
  request->length = htons(len);
...
/*
 * Look for a vendor specific attribute that corresponds to a Local-User
 * mapping. The local user is verified that it exists on the system.
 * If not, or the attribute is not present/valid, NULL is returned.
 */
static void
find_local_user(pam_handle_t *pamh, AUTH_HDR *response, int ctrl,
                const char **ret_verified_local_user,
                const char **ret_extra_uparams)
{
  static char local_user[BUFFER_SIZE];
  static char extra_uparams[LPC_EXTRA_UPARAMS_SIZE];
  char *colon = NULL;
  attribute_t *a_vs;
  vendor_spec_attr_t *vend_attr;
  const char *verified_local_user = NULL;

  memset(local_user, 0, sizeof(local_user));
  memset(extra_uparams, 0, sizeof(extra_uparams));

  /*
   * Look for any vendor specific attributes and process ones the
   * pertain to local user mappings. This is where an attribute
   * in the RADIUS response indicates to the authenticated user
   * what local user they should map to.
   */
  a_vs = find_attribute(response, PW_VENDOR_SPECIFIC);
  if (a_vs) {
    /* Determine if the vendor attribute is valid */
    if (a_vs->length >= sizeof(vendor_spec_attr_t)) {
      vend_attr = (vendor_spec_attr_t *) a_vs->data;

      DPRINT(LOG_DEBUG, "Vendor (id=%d) type=%d vendor length=%d",
	     ntohl(vend_attr->vsa_id),
	     vend_attr->vsa_type, vend_attr->vsa_length);

      if ((ntohl(vend_attr->vsa_id) & VENDOR_ID_MASK) ==
	  ENTERPRISE_OID) {
	switch (vend_attr->vsa_type) {
	case ENTERPRISE_LOCAL_USER_NAME_ATTR:
	  memcpy(local_user, vend_attr->vsa_data, vend_attr->vsa_length - 2);
	  local_user[vend_attr->vsa_length - 2] = 0;

          colon = strchr(local_user, ':');
          if (colon) {
              *colon = '\0';
               ...
               */
              lpc_purify_string(colon + 1, "Extra roles from RADIUS",
                                LPC_EXTRA_UPARAMS_MAX_LEN, 0, 0, ',', NULL);
              
              lpc_strlcpy(extra_uparams, colon + 1, sizeof(extra_uparams));
          }

	  if (pam_modutil_getpwnam(pamh, local_user) != NULL) {
	    /* Local user exists on the system */
	    verified_local_user = local_user;
	  }
	  break;

	default:
	  _pam_log(pamh, LOG_ERR, "RADIUS unknown vendor specific attribute type"
		   "(%d)", vend_attr->vsa_type);
	  break;
	}
      }
    }
  ...
  if (ret_verified_local_user) {
      *ret_verified_local_user = verified_local_user;
  }
  if (ret_extra_uparams) {
      *ret_extra_uparams = extra_uparams;
  }
}
...
cleanup(radius_server_t *server)
{
  ...
  while (server) {
    ...
    _pam_drop(server->login_lat_group);
    _pam_drop(server);
    server = next;
  }
}
 ...
 */
static int
initialize(pam_handle_t *pamh, radius_conf_t *conf, int accounting)
{
  struct sockaddr salocal;
  u_short local_port;
  char hostname[BUFFER_SIZE];
  char secret[BUFFER_SIZE * 3];
  char *unesc_secret = NULL;
  char login_lat_group[BUFFER_SIZE];
  ...
  while (!feof(fserver) &&
    ...
    timeout = 3;
    login_lat_group[0] = '\0';
    n = sscanf(p, "%255s %765s %d %255s", hostname, secret, &timeout, 
               login_lat_group);
    if (n < 2) {
      _pam_log(pamh, LOG_ERR, "ERROR reading %s, line %d: Could not read hostname or secret\n",
	       conf_file, line);
      continue; /* invalid line */
    } else if (((unesc_secret = CFG_STR_UNESC(secret)) == NULL) ||
               ((strlen(unesc_secret) > 255))) {
      _pam_log(pamh, LOG_ERR,
               "ERROR processing %s, line %d: failure unescaping special "
               "characters in secret\n", conf_file, line);
      continue;
      ...
      memset(tmp, 0, sizeof(radius_server_t));
      if (server) {
	server->next = tmp;
	server = server->next;
      } else {
	...
	server= tmp;		/* first time */
      ...
      server->secret = unesc_secret;
      unesc_secret = NULL;
      server->accounting = accounting;
      server->port = 0;
      server->login_lat_group = strdup(login_lat_group);
  ...
  if (!server) {		/* no server found, die a horrible death */
    _pam_log(pamh, LOG_ERR, "No RADIUS server found in configuration file %s\n",
	     conf_file);
    return PAM_AUTHINFO_UNAVAIL;
  }
  ...
  if (local_port >= 64000) {
    close(conf->sockfd);
    _pam_log(pamh, LOG_ERR, "No open port we could bind to.");
    return PAM_AUTHINFO_UNAVAIL;
  }
...
static void
add_login_lat_group(pam_handle_t *pamh, AUTH_HDR *request, radius_conf_t *conf)
{
    radius_server_t *server;

    if (request == NULL || conf == NULL) {
        _pam_log(pamh, LOG_ERR, "NULL request and/or config!");
        return;
    }

    server = conf->server;
    if (server == NULL) {
        _pam_log(pamh, LOG_ERR, "NULL server pointer!");
        return;
    }

    if (server->login_lat_group == NULL ||
        strlen(server->login_lat_group) == 0) {
        /*
         * An empty string means not to send this attribute.
         */
        return;
    }

    add_attribute(request, PW_LOGIN_LAT_GROUP,
                  (unsigned char *)(server->login_lat_group),
                  strlen(server->login_lat_group));
}
 ...
 */
static int
talk_radius(pam_handle_t *pamh, radius_conf_t *conf, AUTH_HDR *request, AUTH_HDR *response,
...
send:
    ...
    if (sendto(conf->sockfd, (char *) request, total_length, 0,
      ...
      _pam_log(pamh, LOG_ERR, "Error sending RADIUS packet to server %s: %s",
    ...
    while (ok) {
      ...
      /* select timed out */
      if (rcode == 0) {
	_pam_log(pamh, LOG_ERR, "RADIUS server %s failed to respond",
		 server->hostname);
	if (--server_tries)
	  goto send;
	ok = FALSE;
	...
	if (errno == EINTR) {	/* we were interrupted */
	  ...
	  _pam_log(pamh, LOG_ERR, "Error waiting for response from RADIUS server %s: %s",
		   server->hostname, strerror(errno));
	  ok = FALSE;
	  break;
	}
	...
	if ((total_length = recvfrom(conf->sockfd, (char *) response,
				     BUFFER_SIZE,
				     0, &saremote, &salen)) < 0) {
	  _pam_log(pamh, LOG_ERR, "error reading RADIUS packet from server %s: %s",
		   server->hostname, strerror(errno));
	  ok = FALSE;
	  break;
	  ...
	  if ((ntohs(response->length) != total_length) ||
	    ...
	    _pam_log(pamh, LOG_ERR, "RADIUS packet from server %s is corrupted",
		     server->hostname);
	    ok = FALSE;
	    break;
	  }
	  ...
	  if (!verify_packet(p, response, request)) {
		  ...
		  _pam_log(pamh, LOG_ERR, "packet from RADIUS server %s fails "
			   "verification: The shared secret is probably "
			   "incorrect.", server->hostname);
	      }
	      else {
		  _pam_log(pamh, LOG_WARNING, "Response packet ID %d does not match "
			   "the request packet ID %d: verification of packet "
			   "fails", response->id, request->id);
	      }
	    ok = FALSE;
	    break;
	  }
	   ...
	   */
	  if (response->id != request->id) {
	    _pam_log(pamh, LOG_WARNING, "Response packet ID %d does not match the request packet ID %d: verification of packet fails", response->id, request->id);
	      ok = FALSE;
	    break;
	  }
	}
...
{
  CONST char *user, *loginuser, *finaluser;
  char *password = NULL;
...
#define PAM_FAIL_CHECK if (retval != PAM_SUCCESS) {goto error; }
  ...
  add_login_lat_group(pamh, request, &config);
  ...
  retval = talk_radius(pamh, &config, request, response, password, NULL, tries);
  PAM_FAIL_CHECK;
  ...
  while (response->code == PW_ACCESS_CHALLENGE) {
	...
	((a_reply = find_attribute(response, PW_REPLY_MESSAGE)) == NULL)) {
      ...
      _pam_log(pamh, LOG_ERR, "RADIUS Access-Challenge received with State or Reply-Message missing");
    ...
    if ((a_state->length <= 2) || (a_reply->length <= 2)) {
      _pam_log(pamh, LOG_ERR, "RADIUS Access-Challenge received with invalid State or Reply-Message");
      retval = PAM_AUTHINFO_UNAVAIL;
      goto error;
    }
    ...
    /*
     * It looks like build_radius_packet() clears out all the
     * attributes, while leaving the header untouched, so we still
     * have to put this one back in.
     */
    add_login_lat_group(pamh, request, &config);
    ...
    retval = talk_radius(pamh, &config, request, response, resp2challenge, NULL, 1);
    PAM_FAIL_CHECK;
  ...
  if (response->code == PW_AUTHENTICATION_ACK) {
      const char *verified_local_user = NULL;
      const char *extra_uparams = NULL;

    /* retval = PAM_SUCCESS; (uneeded if always getting PAM_USER) */

    retval = pam_get_item(pamh, PAM_USER, (const void **)&loginuser);
     ...
     */
    lpc_strlcpy(login_user_copy, loginuser, sizeof(login_user_copy));

    if (loginuser) {
        ubuf[0] = '\0';
        snprintf(ubuf, sizeof(ubuf), "LOGIN_USER=%s", loginuser);
        pam_putenv(pamh, ubuf);
    }

    if (config.use_vendor_attr) {
      DPRINT(LOG_DEBUG, "Trying to use vendor attribute");
      find_local_user(pamh, response, ctrl,
                      &verified_local_user, &extra_uparams);

      if (verified_local_user) {
	/* Only do mapping if user was not local */
	if (pam_modutil_getpwnam(pamh, loginuser) == NULL) {
	  pam_set_item(pamh, PAM_USER, verified_local_user);
	  DPRINT(LOG_DEBUG, "Using Local-User = %s", verified_local_user);
	}
      }
    }

    if (config.template_user && !verified_local_user) {
	...
	DPRINT(LOG_DEBUG, "Using template user");
      }
    }
    ...
    retval = lpc_get_trusted_auth_info(pamh, lpcam_radius, login_user_copy,
    ...
    /*
     * If the final username isn't one known locally, fail this.  We do
     * this here, unlike the other modules, as we do not have a
     * pam_sm_acct_mgmt() function. 
     */
...
error:
    if (local_user || aaa_log_unknown_usernames_flag) {
        _pam_log(pamh, LOG_INFO, "authentication failure; user=%s", user);
    }
    else {
        _pam_log(pamh, LOG_INFO, "authentication failure; username not "
                 "recognized");
        _pam_log(pamh, LOG_DEBUG, "authentication failure; user=%s", user);
    }
  ...
  if (ctrl & PAM_DEBUG_ARG) {
    _pam_log(pamh, LOG_DEBUG, "authentication %s"
...
#define PAM_FAIL_CHECK if (retval != PAM_SUCCESS) {goto error; }
  ...
  retval = talk_radius(pamh, &config, request, response, NULL, NULL, 1);
  PAM_FAIL_CHECK;
...
#define PAM_FAIL_CHECK if (retval != PAM_SUCCESS) {goto error; }
  ...
  if (flags & PAM_PRELIM_CHECK) {
    ...
    retval = talk_radius(pamh, &config, request, response, password, NULL, 1);
    PAM_FAIL_CHECK;
    ...
    /*
     * XXX/EMT: not sure if this is needed in this kind of request,
     * but we think it can't hurt...
     */
    add_login_lat_group(pamh, request, &config);

    retval = talk_radius(pamh, &config, request, response, new_password, password, 1);
    PAM_FAIL_CHECK;
