...
main(int argc, char **argv) {
	...
	if (arg_conf) {
		...
		if ((dhclient_request_options != 0) && (top_level_config.requested_options != (struct option **)default_requested_options)) {
			for (ip=interfaces; ip; ip = ip->next) {
                            if (ip->client->config->requested_options == (struct option **)default_requested_options)
