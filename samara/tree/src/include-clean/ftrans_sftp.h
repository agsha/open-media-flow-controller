...
#ifndef __FTRANS_SFTP_H_
#define __FTRANS_SFTP_H_
...
/* ------------------------------------------------------------------------- */
/** \file ftrans_sftp.h Utilities for transfering files via sftp
 * \ingroup lc
 */
...
/**
 * Transfer a file via sftp (blocking wait).  If pull_file == true, copy
 ...
 * \retval ret_output Error text from sftp
 */
int lft_sftp_transfer_file(tbool pull_file, tbool password_fd,
...
/**
 * Transfer a file via sftp without blocking.  If pull_file == true, copy
 * from remote to local, else from local to remote.  This function should
 * be followed with a call to lft_sftp_transfer_file_non_blocking_complete()
 ...
 */
int lft_sftp_transfer_file_non_blocking(tbool pull_file, tbool password_fd,
                                       const char *remote_user,
                                       const char *remote_password,
                                       const char *remote_host,
                                       uint16 remote_port,
...
/**
 * Completion function companion to lft_sftp_transfer_file_non_blocking() to
 ...
 * by the caller).
 */
int lft_sftp_transfer_file_non_blocking_complete(lc_launch_result **
                                                inout_launch_result,
                                                    lft_state **
                                                inout_ft_state,
                                                int *ret_return_code,
