...
#define max_repetitions errindex
    ...
    /*
     * If set, this flag means this PDU is invalid, and should not 
     * be sent.  It is currently used for marking v1 trap PDUs which
     * contain a Counter64 varbind, to make sure we don't try to 
     * send it.  We have this marker so that we can log about dropping
     * the trap only when we're sure we actually wanted to send it;
     * rather than at construction time, which may just lead to a 
     * false alarm.  (See bug 14522)
     *
     * snmp_pdu_create() allocates a PDU with calloc(), which
     * initializes memory to zero, so we know this will start out
     * cleared, as we want.
     */
    int             tms_invalid;
