/*
 *
 * src/bin/mgmtd/md_utils_internal.h
 *
 *
 *
 */
 ...
 *
 * NOTE: md_route_type_new_map, md_route_type_ipv4_config_map,
 * md_route_type_ipv4_state_map, and md_route_type_old_map
 * (in md_utils_internal.c) must be kept in sync.
 */
typedef enum {
    mrt_none = 0,
    mrt_gw,
...
extern const lc_enum_string_map md_route_type_new_map[];

extern const lc_enum_string_map md_route_type_ipv4_config_map[];

extern const lc_enum_string_map md_route_type_ipv4_state_map[];

extern const lc_enum_string_map md_route_type_old_map[];

extern const char md_if_devsource_unknown[];
...
typedef struct md_route_rec {
     ...
     * appear in different md_route_recs).
     */
    tbool mrr_multipath;
     ...
     * never be set in an md_route_rec returned from
...
int md_route_rec_new(md_route_rec **ret_mrr);

int md_route_rec_free(md_route_rec **inout_rec);

TYPED_ARRAY_HEADER_NEW_NONE(md_route_rec, struct md_route_rec *);

int md_route_rec_array_new(md_route_rec_array **ret_arr);
 ...
 */
int md_route_rec_compare(const md_route_rec *r1, const md_route_rec *r2,
                         tbool devname_special, tbool ignore_devname,
                         tbool ignore_protocol);

/* ------------------------------------------------------------------------- */
/** Read dynamic route information.  Create an array of (md_route_rec *)
 ...
 */
int md_rts_read_route_info(tbool include_ipv4, tbool include_ipv6, 
                           md_route_rec_array **ret_routes);
...
int md_get_interface_networks(md_commit *commit, const mdb_db *db,
                              ...
                              const md_route_rec_array *routes_have,
 ...
 * We look only at one family, either checking for DHCPv4 or DHCPv6.
 */
int md_interface_dhcp_exists(md_commit *commit, const mdb_db *db,
                             tbool ipv6, tbool *ret_exists);
 ...
 * change that.
 *
 * The 'username' parameter here is the name of the user whose account
 * is proposed to be modified.  It is NOT necessarily the name of the 
 * user performing the operation -- that is encapsulated in 'commit'.
 *
 * Specifically, we deny the operation unless:
 *
 *   (the commit structure belongs to 'auth_group_unrestricted' OR
 *    (the commit structure belongs to 'auth_group_superset' AND
...
int md_utils_acl_check_user_commit(md_commit *commit, const mdb_db *old_db,
                                   ...
                                   ta_auth_group auth_group_unrestricted,
                                   ta_auth_group auth_group_superset,
...
typedef struct md_inotify_options md_inotify_options;
typedef struct md_inotify_event md_inotify_event;


/* ------------------------------------------------------------------------- */
/* Callback to be called whenever an Inotify-notifiable event occurs.
 */
typedef int (*md_inotify_callback)(const md_inotify_options *options,
                                   const md_inotify_event *event,
                                   void *data);


/* ------------------------------------------------------------------------- */
/* Registration options structure for setting up an Inotify watch.
 */
struct md_inotify_options {
    /*
     * Name of directory to be watched.  Note that this is not recursive;
     * subdirectories underneath this one are NOT automatically watched.
     */
    const char *mio_path;

    /*
     * The minimum interval of time that should pass between notifications.
     * If another notifiable event happens less than this time period since
     * the last one, we will wait until the period expires and then call
     * the callback.
     */
    lt_dur_ms mio_min_interval;

    /*
     * Mask to select which kinds of events to be called back for.
     * This is passed as the third parameter to inotify_add_watch();
     * see that man page for details.
     */
    uint32 mio_mask;

    /*
     * Callback to be called when one of the specified events occurs.
     */
    md_inotify_callback mio_callback;

    /*
     * Data to be passed to mio_callback whenever it is called.
     */
    void *mio_callback_data;
    
};


/* ------------------------------------------------------------------------- */
/* Default value for mir_min_interval in newly created md_inotify_options
 * structs.
 */
static const lt_dur_ms md_inotify_min_interval_default = 1000;


/* ------------------------------------------------------------------------- */
/* Return an initialized Inotify registration options structure.
 */
int md_inotify_options_create(md_inotify_options **ret_options);


/* ------------------------------------------------------------------------- */
/* Free an Inotify registration options structure.
 */
int md_inotify_options_free(md_inotify_options **inout_options);


/* ------------------------------------------------------------------------- */
/* Event structure passed to md_inotify_callback whenever it is called.
 * This is to allow for future expansion; later we could tell the callback
 * what file was modified, etc.
 */
struct md_inotify_event {
    int mie_dummy; /* Always zero, may be removed later, ignore */
};


/* ------------------------------------------------------------------------- */
/* Register an Inotify watch.  This function takes ownership of the 
 * registration options structure provided.
 */
int md_inotify_register(md_inotify_options **inout_options);
