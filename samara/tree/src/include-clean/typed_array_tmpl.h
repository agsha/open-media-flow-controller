...
#ifndef __TYPED_ARRAY_TMPL_H_
#define __TYPED_ARRAY_TMPL_H_
...
/*
 * XXX64, see comments below.  However,
 * "thinking now is that if a 'void *' is larger, this should work,
 * so the issue reamins if the sizeof(type) is larger than a 'void *'
 * then it won't. Currently we bail if this is detected."
 */

/* ========================================================================= */
/** \file typed_array_tmpl.h Macros to create type-specific arrays.
 * \ingroup lc_ds
 *
 * This file contains macros that can be used to declare new array
 * types that hold a particular data type, rather than a void *.
 ...
 *
 * NOTE: this currently only works with types that are the same size
 * as a void *, which currently means pointers and 32-bit integers.
 * It inherently does not work with larger types because the elements
 * are cast as void *s to be stored in the array.  It does not
 * currently work with smaller types because of the way the foreach
 * function is cast as an array foreach function, though this could be
 * fixed if required. XXX64
 * XXX thinking now is that if a 'void *' is larger, this should work,
 * so the issue remains if the sizeof(type) is larger than a 'void *'
 * then it won't. Currently we bail if this is detected.
 *
 * There are three sets of macros available are as listed below.  For
 * normal header declarations:
 * \li TYPED_ARRAY_HEADER_NEW_DEFAULTS(name, type)
 * \li TYPED_ARRAY_HEADER_NEW_FULL(name, type)
 * \li TYPED_ARRAY_HEADER_NEW_NONE(name, type)
 *
 * For header declarations where you need to split up the type declaration 
 * and the function prototypes:
 * \li TYPED_ARRAY_PRE_DECL(name)
 * \li TYPED_ARRAY_HEADER_BODY_NEW_DEFAULTS(name, type)
 * \li TYPED_ARRAY_HEADER_BODY_NEW_FULL(name, type)
 * \li TYPED_ARRAY_HEADER_BODY_NEW_NONE(name, type)
 *
 * For the implementation:
 * \li TYPED_ARRAY_IMPL_NEW_DEFAULTS(name, type, empty value)
 * \li TYPED_ARRAY_IMPL_NEW_FULL(name, type, empty value)
 * \li TYPED_ARRAY_IMPL_NEW_NONE(name, type, empty value)
 *
 * To declare a new array type:
 *
 * \li Call one of the TYPED_ARRAY_HEADER... variants from your header
 * file.  For example, TYPED_ARRAY_HEADER_NEW_NONE(str, char *) was
 ...
 * array options.
 *
 * \li Call one of the TYPED_ARRAY_IMPL... variants from your .c file.
 ...
 * TYPED_ARRAY_PRE_DECL for that, and then use a
 * TYPED_ARRAY_HEADER_BODY... variant for the remainder of the header.
 *
...
#define TYPED_ARRAY_PRE_DECL(NAME)                                            \
                                                                              \
typedef struct NAME ## _array NAME ## _array;

#define TYPED_ARRAY_NEW_FULL_H(NAME)                                          \
                                                                              \
    int NAME ## _array_new(NAME ## _array **ret_arr,                          \
                           const array_options *opts);

#define TYPED_ARRAY_NEW_DEFAULTS_H(NAME)                                      \
                                                                              \
    int NAME ## _array_new(NAME ## _array **ret_arr);                         \
                                                                              \
...
#define TYPED_ARRAY_NEW_NONE_H(NAME)                                          \
                                                                              \
    int NAME ## _array_new_full(NAME ## _array **ret_arr,                     \
                                const array_options *opts);

#define TYPED_ARRAY_HEADER_BODY(NAME, TYPE)                                   \
                                                                              \
struct NAME ## _array {                                                       \
    array * NAME ## _arr;                                                     \
};                                                                            \
...
int NAME ## _array_equal(const NAME ## _array *arr1,                          \
                         const NAME ## _array *arr2, tbool *ret_equal);       \
                                                                              \
int NAME ## _array_equal_sparse(const NAME ## _array *arr1,                   \
                         const NAME ## _array *arr2, tbool *ret_equal);       \
                                                                              \
...
int NAME ## _array_set(NAME ## _array *arr, uint32 idx, const TYPE elem);     \
                                                                              \
int NAME ## _array_set_takeover(NAME ## _array *arr, uint32 idx, TYPE *elem); \
                                                                              \
int NAME ## _array_append(NAME ## _array *arr, const TYPE elem);              \
                                                                              \
int NAME ## _array_append_takeover(NAME ## _array *arr, TYPE *elem);          \
                                                                              \
int NAME ## _array_insert(NAME ## _array *arr, uint32 idx, const TYPE elem);  \
                                                                              \
int NAME ## _array_insert_takeover(NAME ## _array *arr,                       \
                                   uint32 idx,                                \
                                   TYPE *elem);                               \
                                                                              \
int NAME ## _array_insert_sorted(NAME ## _array *arr, const TYPE elem);       \
                                                                              \
int NAME ## _array_insert_sorted_idx(NAME ## _array *arr, const TYPE elem,    \
                                     uint32 *ret_insert_idx);                 \
                                                                              \
int NAME ## _array_insert_sorted_takeover(NAME ## _array *arr,                \
                                          TYPE *elem);                        \
                                                                              \
int NAME ## _array_insert_sorted_hint(NAME ## _array *arr, const TYPE elem,   \
                                      uint32 hint_index);                     \
                                                                              \
int NAME ## _array_insert_sorted_hint_idx(NAME ## _array *arr,                \
                                          const TYPE elem, uint32 hint_index, \
                                          uint32 *ret_insert_idx);            \
                                                                              \
int NAME ## _array_insert_sorted_hint_takeover(NAME ## _array *arr,           \
                                               TYPE *elem,                    \
                                               uint32 hint_index);            \
                                                                              \
...
int NAME ## _array_push(NAME ## _array *arr, const TYPE elem);                \
                                                                              \
int NAME ## _array_push_takeover(NAME ## _array *arr, TYPE *elem);            \
                                                                              \
...
#define TYPED_ARRAY_HEADER_BODY_NEW_DEFAULTS(NAME, TYPE)                      \
        TYPED_ARRAY_NEW_DEFAULTS_H(NAME)                                      \
        TYPED_ARRAY_HEADER_BODY(NAME, TYPE)

#define TYPED_ARRAY_HEADER_BODY_NEW_FULL(NAME, TYPE)                          \
        TYPED_ARRAY_NEW_FULL_H(NAME)                                          \
        TYPED_ARRAY_HEADER_BODY(NAME, TYPE)

#define TYPED_ARRAY_HEADER_BODY_NEW_NONE(NAME, TYPE)                          \
        TYPED_ARRAY_NEW_NONE_H(NAME)                                          \
        TYPED_ARRAY_HEADER_BODY(NAME, TYPE)

#define TYPED_ARRAY_HEADER_NEW_DEFAULTS(NAME, TYPE)                           \
        TYPED_ARRAY_PRE_DECL(NAME)                                            \
        TYPED_ARRAY_HEADER_BODY_NEW_DEFAULTS(NAME, TYPE)

#define TYPED_ARRAY_HEADER_NEW_FULL(NAME, TYPE)                               \
        TYPED_ARRAY_PRE_DECL(NAME)                                            \
        TYPED_ARRAY_HEADER_BODY_NEW_FULL(NAME, TYPE)

#define TYPED_ARRAY_HEADER_NEW_NONE(NAME, TYPE)                               \
        TYPED_ARRAY_PRE_DECL(NAME)                                            \
        TYPED_ARRAY_HEADER_BODY_NEW_NONE(NAME, TYPE)
...
#define TYPED_ARRAY_NEW_FULL_C(NAME, TYPE)                                    \
                                                                              \
int NAME ## _array_new_full(NAME ## _array **ret_arr,                         \
                            const array_options *opts)                        \
{                                                                             \
    ...
    uint32 size_needed = 0;                                                   \
                                                                              \
    bail_null(ret_arr);                                                       \
    bail_require(sizeof(TYPE) <= sizeof(void *));                             \
                                                                              \
...
#define TYPED_ARRAY_NEW_OPTS_C(NAME, TYPE)                                    \
                                                                              \
int NAME ## _array_new(NAME ## _array **ret_arr,                              \
                       const array_options *opts)                             \
{                                                                             \
...
#define TYPED_ARRAY_NEW_DEFAULTS_C(NAME, TYPE)                                \
                                                                              \
int NAME ## _array_new(NAME ## _array **ret_arr)                              \
{                                                                             \
    int err = 0;                                                              \
...
#define TYPED_ARRAY_IMPL(NAME, TYPE, EMPTY_VALUE)                             \
                                                                              \
int NAME ## _array_options_get(NAME ## _array *arr,                           \
                               array_options *ret_options)                    \
...
int NAME ## _array_equal_sparse(const NAME ## _array *arr1,                   \
                         const NAME ## _array *arr2, tbool *ret_equal)        \
{ IF_EITHER_NON_NULL(arr1, arr2,                                              \
  return(array_equal_sparse(arr1-> NAME ## _arr,                              \
                            arr2-> NAME ## _arr,  ret_equal))); }             \
                                                                              \
int NAME ## _array_get                                                        \
    (const NAME ## _array *arr, uint32 idx, TYPE *ret_elem)                   \
{ IF_NON_NULL(arr, return(array_get(arr-> NAME ## _arr,                       \
              idx, (void **)ret_elem))) }                                     \
                                                                              \
TYPE NAME ## _array_get_quick(const NAME ## _array *arr, uint32 idx)          \
{ IF_NON_NULL_SPECIAL(arr, EMPTY_VALUE, return((TYPE)array_get_quick          \
                      (arr-> NAME ## _arr, idx))) }                           \
                                                                              \
int NAME ## _array_set(NAME ## _array *arr, uint32 idx, const TYPE elem)      \
{ IF_NON_NULL(arr, return(array_set(arr-> NAME ## _arr, idx,                  \
              (void *)elem))) }                                               \
                                                                              \
int NAME ## _array_set_takeover(NAME ## _array *arr, uint32 idx, TYPE *elem)  \
{ IF_NON_NULL(arr, return(array_set_takeover(arr-> NAME ## _arr, idx,         \
              (void **)elem))) }                                              \
                                                                              \
int NAME ## _array_append(NAME ## _array *arr, const TYPE elem)               \
{ IF_NON_NULL(arr, return(array_append(arr-> NAME ## _arr, (void *)elem))) }  \
                                                                              \
int NAME ## _array_append_takeover(NAME ## _array *arr, TYPE *elem)           \
{ IF_NON_NULL(arr, return(array_append_takeover(arr-> NAME ## _arr,           \
                                                (void **)elem))) }            \
                                                                              \
int NAME ## _array_insert(NAME ## _array *arr, uint32 idx, const TYPE elem)   \
{ IF_NON_NULL(arr, return(array_insert(arr-> NAME ## _arr, idx,               \
              (void *)elem))) }                                               \
                                                                              \
int NAME ## _array_insert_takeover(NAME ## _array *arr, uint32 idx,           \
                                   TYPE *elem)                                \
{ IF_NON_NULL(arr, return(array_insert_takeover(arr-> NAME ## _arr, idx,      \
              (void **)elem))) }                                              \
                                                                              \
int NAME ## _array_insert_sorted(NAME ## _array *arr, const TYPE elem)        \
...
int NAME ## _array_insert_sorted_idx(NAME ## _array *arr, const TYPE elem,    \
                                     uint32 *ret_insert_idx)                  \
{ IF_NON_NULL(arr, return(array_insert_sorted_idx(arr-> NAME ## _arr,         \
              (void *)elem, ret_insert_idx))) }                               \
                                                                              \
int NAME ## _array_insert_sorted_takeover(NAME ## _array *arr,                \
                                          TYPE *elem)                         \
{ IF_NON_NULL(arr, return(array_insert_sorted_takeover(arr-> NAME ## _arr,    \
              (void **)elem))) }                                              \
                                                                              \
int NAME ## _array_insert_sorted_hint(NAME ## _array *arr, const TYPE elem,   \
                                      uint32 hint_index)                      \
{ IF_NON_NULL(arr, return(array_insert_sorted_hint                            \
              (arr-> NAME ## _arr, (void *)elem, hint_index))) }              \
                                                                              \
int NAME ## _array_insert_sorted_hint_idx(NAME ## _array *arr,                \
                                          const TYPE elem, uint32 hint_index, \
                                          uint32 *ret_insert_idx)             \
{ IF_NON_NULL(arr, return(array_insert_sorted_hint_idx                        \
              (arr-> NAME ## _arr, (void *)elem, hint_index,                  \
               ret_insert_idx))) }                                            \
                                                                              \
int NAME ## _array_insert_sorted_hint_takeover(NAME ## _array *arr,           \
                                               TYPE *elem,                    \
                                               uint32 hint_index)             \
{ IF_NON_NULL(arr, return(array_insert_sorted_hint_takeover                   \
              (arr-> NAME ## _arr, (void **)elem, hint_index))) }             \
                                                                              \
...
{ IF_NON_NULL(arr, return(array_delete_no_shift(arr-> NAME ## _arr, idx))) }  \
                                                                              \
int NAME ## _array_remove(NAME ## _array *arr, uint32 idx, TYPE *ret_elem)    \
{ IF_NON_NULL(arr, return(array_remove(arr-> NAME ## _arr, idx,               \
              (void **)ret_elem))) }                                          \
                                                                              \
int NAME ## _array_remove_no_shift(NAME ## _array *arr, uint32 idx,           \
                                   TYPE *ret_elem)                            \
{ IF_NON_NULL(arr, return(array_remove_no_shift                               \
                          (arr-> NAME ## _arr, idx, (void **)ret_elem))) }    \
                                                                              \
int NAME ## _array_delete_by_value(NAME ## _array *arr, TYPE elem,            \
                                   tbool allow_multiple)                      \
{ IF_NON_NULL(arr, return(array_delete_by_value                               \
              (arr-> NAME ## _arr, (void *)elem, allow_multiple))) }          \
                                                                              \
int NAME ## _array_remove_by_value(NAME ## _array *arr, TYPE elem,            \
                                   TYPE *ret_elem)                            \
{ IF_NON_NULL(arr, return(array_remove_by_value                               \
              (arr-> NAME ## _arr, (void *)elem, (void **)ret_elem))) }       \
                                                                              \
int NAME ## _array_delete_range(NAME ## _array *arr, uint32 idx,              \
                                int32 num_elems, tbool shift_to_cover)        \
{ IF_NON_NULL(arr, return(array_delete_range(arr-> NAME ## _arr, idx,         \
                                             num_elems, shift_to_cover))) }   \
                                                                              \
int NAME ## _array_push(NAME ## _array *arr, const TYPE elem)                 \
{ IF_NON_NULL(arr, return(array_push(arr-> NAME ## _arr, (void *)elem))) }    \
                                                                              \
int NAME ## _array_push_takeover(NAME ## _array *arr, TYPE *elem)             \
{ IF_NON_NULL(arr, return(array_push_takeover(arr-> NAME ## _arr,             \
                                              (void **)elem))) }              \
                                                                              \
int NAME ## _array_pop(NAME ## _array *arr, TYPE *ret_elem)                   \
{ IF_NON_NULL(arr, return(array_pop(arr-> NAME ## _arr,                       \
                          (void **)ret_elem))) }                              \
                                                                              \
int NAME ## _array_peek(const NAME ## _array *arr, TYPE *ret_elem)            \
{ IF_NON_NULL(arr, return(array_peek(arr-> NAME ## _arr,                      \
                          (void **)ret_elem))) }                              \
                                                                              \
int NAME ## _array_sort(NAME ## _array *arr)                                  \
...
{ IF_NON_NULL(arr, return(array_binary_search(arr-> NAME ## _arr,             \
                          (void *)model_elem, ret_found_index))) }            \
                                                                              \
...
{ IF_NON_NULL(arr, return(array_binary_search_hint(arr-> NAME ## _arr,        \
    (void *)model_elem, hint_index, ret_found_index))) }                      \
                                                                              \
...
{ IF_NON_NULL(arr, return(array_linear_search(arr-> NAME ## _arr,             \
     (void *)model_elem, start_index, ret_found_index))) }                    \
                                                                              \
...
{ IF_NON_NULL(arr, return(array_linear_search_custom(arr-> NAME ## _arr,      \
   (void *)model_elem, custom_compare_func, start_index, ret_found_index))) } \
                                                                              \
int NAME ## _array_best_search(const NAME ## _array *arr,                     \
                               const TYPE model_elem,                         \
                               uint32 *ret_found_index)                       \
...
{                                                                             \
     ...
     * _array_new functions (which check if sizeof(void *) greater            \
     * than or equal to sizeof(TYPE).                                         \
     */                                                                       \
    bail_require(fe_args->afsa_arr-> NAME ## _arr == arr);                    \
    err = (fe_args->afsa_func)(fe_args->afsa_arr, idx,                        \
                               (TYPE) elem, fe_args->afsa_data);              \
bail:                                                                         \
    return(err);                                                              \
}

#define TYPED_ARRAY_IMPL_NEW_DEFAULTS(NAME, TYPE, EMPTY_VALUE)                \
        TYPED_ARRAY_NEW_DEFAULTS_C(NAME, TYPE)                                \
        TYPED_ARRAY_NEW_FULL_C(NAME, TYPE)                                    \
        TYPED_ARRAY_IMPL(NAME, TYPE, EMPTY_VALUE)

#define TYPED_ARRAY_IMPL_NEW_FULL(NAME, TYPE, EMPTY_VALUE)                    \
        TYPED_ARRAY_NEW_OPTS_C(NAME, TYPE)                                    \
        TYPED_ARRAY_IMPL(NAME, TYPE, EMPTY_VALUE)

#define TYPED_ARRAY_IMPL_NEW_NONE(NAME, TYPE, EMPTY_VALUE)                    \
        TYPED_ARRAY_NEW_FULL_C(NAME, TYPE)                                    \
        TYPED_ARRAY_IMPL(NAME, TYPE, EMPTY_VALUE)
