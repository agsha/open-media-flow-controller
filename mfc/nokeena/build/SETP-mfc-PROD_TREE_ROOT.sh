...
#   When the current directory is a MFC source, then
#      Use this source tree
#   Else If PROD_CUSTOMER_ROOT is set and is a existing directory
#      Use source tree $PROD_CUSTOMER_ROOT
#   Else
#      Use the MFC mainline
# When options are specifed:
#   [directory where MFC source is located | branch name | svn url of MFC source] [svn revision]
#
# 
...
MY_REVISION=top
CR_SVN_URL=unset
# The script /eng/bin-svn/mfc_branch_name_from_version.sh returns the
# MFC branch name for a given the release version.
# The script /eng/bin-svn/mfc_svn_url_from_branch.sh returns the svn URL
# for the PROD_CUSTOMER_ROOT source for a given svn branch name.
# 
if [ -z "${1}" ] ; then
  echo "Error, parameter reqired"
  echo Required first parameter specifies the MFC source.  One of the following:
  echo '  -' directory_where_MFC_source_is_located
  echo '  -' branch_name
  echo '  -' MFC version, e.g. 12.3.0
  echo '  -' svn_url_of_MFC_source
  ...
  if [ -d ${OPT1} ] ; then
    ...
    svn://*)
      ...
      ;;
    0*|1*|2*|3*|4*)
      BR_NAME=`/eng/bin-svn/mfc_branch_name_from_version.sh ${OPT1} ${MY_REVISION}`
      CR_SVN_URL=`/eng/bin-svn/mfc_svn_url_from_branch.sh ${BR_NAME}`
      ;;
    *)
      CR_SVN_URL=`/eng/bin-svn/mfc_svn_url_from_branch.sh ${OPT1}`
      ;;
    esac
  fi
fi
if [ "${CR_SVN_URL}" = "unset" ] ; then
  # First see if we are in a MFC source tree or not.
  CR_SVN_URL=`svn info 2> /dev/null | grep URL: | cut -f2 -d' ' | cut -f1-7 -d/`
  if [ -z "${CR_SVN_URL}" ] ; then
    if [ -z "${PROD_CUSTOMER_ROOT}" ] ; then
      echo "Use MFC mainline" 1>&2
      CR_SVN_URL=`/eng/bin-svn/mfc_svn_url_from_branch.sh trunk`
...
Get_It()
{
  ...
  if [ "${MY_REVISION}" = "top" ] ; then 
    svn checkout ${CR_SVN_URL}/nokeena/src/mk > /dev/null 2>&1
  else
    svn checkout -r ${MY_REVISION} ${CR_SVN_URL}/nokeena/src/mk >>  /dev/null 2>&1
  fi
