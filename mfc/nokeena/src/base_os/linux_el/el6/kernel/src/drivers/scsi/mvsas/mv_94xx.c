...
static msc_led_t *msc_led;

static void mvs_94xx_set_activity_led_orange(void)
{
	void __iomem *regs = msc_led->second_marvel_regs;
	u32 tmp;

	if(regs) {
		tmp = mr32(MVS_GPIOF_OV);
		tmp |= 0x03;
		mw32(MVS_GPIOF_OV, tmp);
	}
}

static void mvs_94xx_set_activity_led_off(void)
{
	void __iomem *regs = msc_led->second_marvel_regs;
	u32 tmp;
...
static void mvs_94xx_set_activity_led_green(void)
{
	void __iomem *regs = msc_led->second_marvel_regs;
	u32 tmp;
...
static void mvs_94xx_set_activity_led_red(void)
{
	void __iomem *regs = msc_led->second_marvel_regs;
	u32 tmp;
...
static int mvs_94xx_activity_led_on = 0;

static void mvs_94xx_stop_activity_led(void)
{
	mvs_94xx_set_activity_led_off();
	mod_timer(&msc_led->mvs_94xx_led_timer, jiffies + (600 * HZ));
}

static void mvs_94xx_start_activity_led(void)
{
	switch(msc_led->app_led_value) {
		case 1:
			mvs_94xx_set_activity_led_red();
			break;
		case 2:
			mvs_94xx_set_activity_led_orange();
			break;
		case 3:
			mvs_94xx_set_activity_led_green();
			break;
	}
	/* Do not reset in diag mode 4,5 */
	if(msc_led->app_led_value <= 3) {
	    mvs_94xx_activity_led_on = 1;
	    mod_timer(&msc_led->mvs_94xx_led_timer, jiffies + ((4 * HZ) / 10));
	}
}

static void mvs_94xx_set_cpu_led_orange(void)
{
	void __iomem *regs = msc_led->first_marvel_regs;
	u32 tmp;
...
static void mvs_94xx_set_cpu_led_green(void)
{
	void __iomem *regs = msc_led->first_marvel_regs;
	u32 tmp;
...
static void mvs_94xx_set_cpu_led_off(void)
{
	void __iomem *regs = msc_led->first_marvel_regs;
	u32 tmp;
...
static void mvs_94xx_set_cpu_led_red(void)
{
	void __iomem *regs = msc_led->first_marvel_regs;
	u32 tmp;
...
static int pacifica_msc_gpio_write_app_led(struct file *file, const char *buffer,
				unsigned long count, void *data)
{
    ...
    msc_led->app_led_value = proc_value;
    if(proc_value  > 3) {
	switch(proc_value) {
	    case 4:
		mvs_94xx_set_activity_led_red();
...
static int pacifica_msc_gpio_read_app_led(char *buf, char **start, off_t off,
			       int count, int *eof, void *data)
{
    buf[0] = msc_led->app_led_value + 0x30;
    buf[1] = '\0';
    return 2;

}

static int pacifica_msc_gpio_write_cpu_led(struct file *file, const char *buffer,
				unsigned long count, void *data)
{
    int proc_value;

    proc_value = simple_strtoul(buffer,NULL,10);
    msc_led->cpu_led_value = proc_value;
    switch(proc_value) {
	case 0:
	    mvs_94xx_set_cpu_led_off();
	    break;
	case 1:
	    mvs_94xx_set_cpu_led_red();
	    break;
	case 2:
	    mvs_94xx_set_cpu_led_orange();
	    break;
	case 3:
	    mvs_94xx_set_cpu_led_green();
	    break;
    }
    return count;
}

static int pacifica_msc_gpio_read_cpu_led(char *buf, char **start, off_t off,
			       int count, int *eof, void *data)
{
    buf[0] = msc_led->cpu_led_value + 0x30;
    buf[1] = '\0';
    return 2;
...
/*
 * During a shutdown/reboot, the CPU LED will be turned red and the activity LED
 * will be switched off.
 *
 */
static int mvsas_led_notify_sys(struct notifier_block *this, unsigned long code,
			  void *unused)
{
	if (code == SYS_DOWN || code == SYS_HALT) {
		printk(KERN_CRIT "mvsas: system shutdown/reboot\n");
		mvs_94xx_set_cpu_led_red();
		mvs_94xx_stop_activity_led();
		msc_led->first_marvel_regs = 0;
		msc_led->second_marvel_regs = 0;
	}
	return NOTIFY_DONE;
}

static struct notifier_block mvsas_led_notifier = {
	.notifier_call = mvsas_led_notify_sys,
};


static void mvsas_led_init(struct mvs_info *mvi)
{
	u32 tmp;
	void __iomem *regs = mvi->regs_ex - 0x200;
	struct proc_dir_entry *gpio_entry;
	int first_load;

	if(!msc_led) {
		msc_led = kmalloc(sizeof(msc_led_t), GFP_KERNEL);
		if(!msc_led)
			return;
		msc_led->first_marvel_bus_number = 0;
		msc_led->second_marvel_bus_number = 0;
		msc_led->first_marvel_regs = NULL;
		msc_led->second_marvel_regs = NULL;
		init_timer(&msc_led->mvs_94xx_led_timer);
		msc_led->mvs_94xx_led_timer.function =
			(void (*) (unsigned long))mvs_94xx_stop_activity_led;
		msc_led->mvs_94xx_led_timer.data    = 0;
	}

	first_load = 0;
#ifdef DEBUG
	dev_printk(KERN_INFO, mvi->dev, "bus number : = %x\n", mvi->pdev->bus->number);
#endif

	if(!msc_led->first_marvel_bus_number) {
		msc_led->first_marvel_bus_number = mvi->pdev->bus->number;
		msc_led->first_marvel_regs = regs;
		first_load = 1;
	} else if(!msc_led->second_marvel_bus_number) {
	    if(mvi->pdev->bus->number != msc_led->first_marvel_bus_number) {
		msc_led->second_marvel_bus_number = mvi->pdev->bus->number;
		msc_led->second_marvel_regs = regs;
	    }
	}

#ifdef DEBUG
	dev_printk(KERN_INFO, mvi->dev, "first marvel bus number : = %x\n", msc_led->first_marvel_bus_number);
	dev_printk(KERN_INFO, mvi->dev, "second marvel bus number : = %x\n", msc_led->second_marvel_bus_number);
#endif

#ifdef DEBUG
	tmp = mr32(MVS_CHIP_ID);
	dev_printk(KERN_INFO, mvi->dev, "mvsas: Chip id = %x\n", tmp);
#endif

	/* Enable the GPIO 0,1 */
	tmp = mr32(MVS_GPIOF_OC);
	tmp |= 0x03;
	mw32(MVS_GPIOF_OC, tmp);

	/* Set the GPIO 0,1 Output Enable */
	tmp = mr32(MVS_GPIOF_OE);
	tmp |= 0x03;
	mw32(MVS_GPIOF_OE, tmp);

	mvs_94xx_set_cpu_led_green();
	mvs_94xx_set_activity_led_off();
	msc_led->cpu_led_value = 3;
	msc_led->app_led_value = 3;

	if(first_load) {
		/* create cfm directory */
		msc_led->gpio_dir      = proc_mkdir("msc-gpio", NULL);

		gpio_entry             = create_proc_entry("app-led",0600,
						   msc_led->gpio_dir);
		gpio_entry->nlink      = 1;
		gpio_entry->read_proc  = pacifica_msc_gpio_read_app_led;
		gpio_entry->write_proc = pacifica_msc_gpio_write_app_led;
		gpio_entry->data       = NULL;

		gpio_entry             = create_proc_entry("cpu-led",0600,
							msc_led->gpio_dir);
		gpio_entry->nlink      = 1;
		gpio_entry->read_proc  = pacifica_msc_gpio_read_cpu_led;
		gpio_entry->write_proc = pacifica_msc_gpio_write_cpu_led;
		gpio_entry->data       = NULL;

		msc_led->mvs_94xx_led_timer.expires = jiffies + (600 * HZ);
		add_timer(&msc_led->mvs_94xx_led_timer);
		register_reboot_notifier(&mvsas_led_notifier);
	}
