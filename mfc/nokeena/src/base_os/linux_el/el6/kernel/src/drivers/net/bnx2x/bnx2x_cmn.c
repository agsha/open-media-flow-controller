...
#ifdef HAVE_PF_RING
		{
			...
			if (hook && (hook->magic == PF_RING)) {
				...
				if (*hook->transparent_mode != standard_linux_path) {
					...
					u16 ether_type = ntohs(eth->h_proto);
					...
					if (ether_type != ETH_P_IP) {
						...
						if (ntohs(iph->tot_len) == ((iph->ihl * 4) + (tcph->doff * 4)))
							skip_send_up = 1;
						if (debug)
						    printk(KERN_INFO "[2] [PF_RING] ip packet [proto %d %d][len=%d ihl %d doff %d]\n",
							ether_type, iph->protocol, iph->tot_len, iph->ihl, tcph->doff);
					}
