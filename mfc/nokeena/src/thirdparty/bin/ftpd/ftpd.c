...
void douser(const char *username)
{
    ...
    if (anon_only <= 0 && username != NULL && *username != 0 &&
        ...
        /* The username is of the form <namespace>/<userid>
         * We need to get the raw user-id before proceeding
         */
        //char *u_name = strstr(username, "/");
        //if (u_name==NULL) {
            //die(530, LOG_DEBUG, MSG_UNKNOWN_COMMAND);
        //}
        strncpy(account, username, sizeof(account) - 1);
        //strncpy(account, u_name+1, sizeof(account) - 1);
                        ...
                        const char * const name, int up)
{
    ...
    if ( up ) {
        filename = (char *)calloc(sizeof(char),strlen(account)+strlen(".info")+1);
        if ( filename == NULL ) {
            die_mem();
        }
        sprintf(filename,"%s%s",account,".info");
        if ((fp = fopen(filename, "a")) != NULL) {
            fprintf(fp,"%s %8lu %s %s\n", m, 
                   st.st_size, outstr!=NULL?outstr:"", name);
            fclose(fp);
        }
        free(filename);
    }
