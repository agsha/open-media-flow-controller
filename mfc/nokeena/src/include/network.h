...
typedef struct nm_threads {
	...
	uint64_t num;		// This thread number, counting from 0
...
static inline void
NM_set_socket_active(network_mgr_t *pnm)
{
	int slot;
 ...
 */
static inline void
int_NM_close_socket(int fd, int reset_close)
{
	network_mgr_t * pnm;
	...
	if(net_use_nkn_stack) { so_shutdown(fd, 2); }
        else { /*shutdown(fd, SHUT_WR);*/ 
		if (reset_close || close_socket_by_RST_enable) {
...
#define NM_close_socket(_fd) int_NM_close_socket((_fd), 0)
#define NM_reset_close_socket(_fd) int_NM_close_socket((_fd), 1)
