...
extern int nkn_strcmp_case(const char * p1, const char * p2, int n);
...
NKNCNT_DEF( tot_http_resp_vary, uint64_t, "", "Total HTTP response with vary header")
...
NKNCNT_DEF( tunnel_req_incapable_byte_range_origin, uint64_t, "", "tunnel")
...
static int get_vary_values(http_cb_t * phttp, const char *p)
{
	int check = 0;
	int len = 0;
	const char *ps, *p_end;

	SET_HTTP_FLAG2(phttp, HRF2_HAS_VARY);
	ps = nkn_skip_colon_check(p+http_known_headers[MIME_HDR_VARY].namelen, &check);
	p_end = nkn_find_hdrvalue_end(ps);
	len = p_end - ps + 1;
	while (len > 0 && (*ps == ' ' || *ps == '\t')) ps++, len--;
	while (len > 0) {
		if (len && *ps == '*') {
			ps++, len--;
			SET_HTTP_FLAG2(phttp, HRF2_HAS_VARY_OTHERS);
			break;
		} else {
			switch (*(unsigned int *)ps | 0x20202020) {
			case sig_user_agent:
				if (nkn_strcmp_incase(ps, "user-agent", 10) == 0) {
					SET_HTTP_FLAG2(phttp, HRF2_HAS_VARY_USR_AGNT);
					ps += 10;
					len -= 10;
				}
				break;
			case sig_accept_encoding:
				if (nkn_strcmp_incase(ps, "accept-encoding", 15) == 0) {
					SET_HTTP_FLAG2(phttp, HRF2_HAS_VARY_ACCPT_ENCD);
					ps += 15;
					len -= 15;
				}
				break;
...
#undef CHECK_HEX_ENC
		...
		default:
			...
			if (http_allowed_methods.all_methods) {
				...
				if (unk_req_len > 0 ) {
					if ((p[unk_req_len] != ' ') ||
					    (unk_req_len > 8 /* Max Method Length "CONNECT" */)) {
...
HTTP_PARSE_STATUS http_parse_header(http_cb_t * phttp)
{
	...
	while(len>=2) {
		...
		if (CHECK_HTTP_FLAG(phttp, HRF_HTTP_RESPONSE)) {
			phttp->tunnel_reason_code = NKN_TR_RES_MAX_SIZE_LIMIT;
		} else {
			phttp->tunnel_reason_code = NKN_TR_REQ_MAX_SIZE_LIMIT;
			gettimeofday(&(phttp->start_ts), NULL);
		}
...
complete: 
	phttp->http_hdr_len = p-phttp->cb_buf;
        if (phttp->http_hdr_len >= MAX_HTTP_HEADER_SIZE - 10) {
		phttp->http_hdr_len = phttp->cb_totlen;
		if (CHECK_HTTP_FLAG(phttp, HRF_HTTP_RESPONSE)) {
			phttp->tunnel_reason_code = NKN_TR_RES_MAX_SIZE_LIMIT;
		} else {
	...
	// Mark the start time of this HTTP transaction
	if (!CHECK_HTTP_FLAG(phttp, HRF_HTTP_RESPONSE)) {
		gettimeofday(&(phttp->start_ts), NULL);
	}
	...
	if((int)glob_tot_get_in_this_second > max_get_rate_limit) {
		...
		DBG_LOG(MSG, MOD_HTTP, 
			...
			" glob_tot_get_in_this_second=%ld", glob_tot_get_in_this_second);
	...
	while(1) 
	{
		...
		case sig_connection:
			...
			if(nkn_strcmp_incase(p, 
				...
				hit++;
				    ...
				    else if(nkn_strcmp_incase(p, "upgrade", 7)==0) {
					SET_HTTP_FLAG2(phttp, HRF2_CONNECTION_UPGRADE);
				    }
                                    if (ptmp) {
		...
		case sig_transfer_encoding:
			// Tranfer-Encoding: chunked
				...
				p_end = nkn_find_hdrvalue_end(p);
                                /* From RFC-2616: 
                                 *      When the "chunked" transfer- coding is 
                                 *      used, it MUST be the last transfer-coding 
                                 *      applied to the message-body. The "chunked" 
                                 *      transfer-coding MUST NOT be applied more 
                                 *      than once to a message-body.
                                 */
				if(nkn_strcmp_incase(p_end-6, "chunked", 7)==0) {
					SET_HTTP_FLAG(phttp, HRF_TRANSCODE_CHUNKED);
					/* If the value of 'Transfer-Encoding' header starts
                                         * with a sub-encoding type for a chunked object, set this flag.
                                     	*/
					if(p != p_end-6) {
						SET_HTTP_FLAG2(phttp, HRF2_TRANSCODE_CHUNKED_WITH_SUB) ;
					}
				}
		...
		case sig_upgrade:
			{
				...
				http_header_id id = MIME_HDR_UPGRADE;

				tp = nkn_find_hdrname_end(p_start);
				if ((tp-p_start+1) != http_known_headers[id].namelen) break;

				if (nkn_strcmp_incase(p_start,
					    http_known_headers[id].name,
					    http_known_headers[id].namelen)==0) {
					p_start = nkn_skip_colon_check(p_start+http_known_headers[id].namelen, &check);
					if (!check) break;
					p_end = nkn_find_hdrvalue_end(p_start);
					while (*p_end == ' ' || *p_end == '\t' || *p_end == '\r') p_end--;
					if (((*p_end == ':') || (p_start-1) <= p_end) && (*p != '\n')) {
						add_known_header(hdr, id, p_start, ((*p_end == ':') ? 0: (p_end-p_start+1)));
						hit++;
						if (nkn_strcmp_incase(p_start, "websocket", 9)==0) {
							SET_HTTP_FLAG2(phttp, HRF2_UPGRADE_WEBSOCKET);
						}
					}
				}
			}
		...
		case sig_user_agent:
			...
			break;
		case sig_vary:
			glob_tot_http_resp_vary++;
			add_kheader(hdr, MIME_HDR_VARY, p, &hit);
			get_vary_values(phttp, p);
			break;
		...
		case sig_www_authenticate:
			{
				...
				http_header_id id = MIME_HDR_WWW_AUTHENTICATE;
				...
				if (nkn_strcmp_incase(p_start,
					...
					if (((*p_end == ':') || (p_start-1) <= p_end) && (*p != '\n')) {
						add_known_header(hdr, id, p_start, ((*p_end == ':') ? 0: (p_end-p_start+1)));
						hit++;
						if (nkn_strcmp_case(p_start, "NTLM ", 5)==0) {
							SET_HTTP_FLAG(phttp, HRF_WWW_AUTHENTICATE_NTLM);
						}
					}
				}
			}
			...
			if (phttp->nsconf) {
				...
				if (hdr_len) {
					...
					add_if_custom_name_match(hdr, MIME_HDR_X_ACCEL_CACHE_CONTROL, p, 
				  	...
				  	MIME_HDR_X_NKN_CLUSTER_TPROXY, p, &hit)) {
				  } else if (add_if_match(hdr, 
...
int http_check_mfd_limitation(http_cb_t * phttp, uint64_t pe_action)
{
	...
	if (CHECK_HTTP_FLAG(phttp, HRF_TRANSCODE_CHUNKED)) {
		CLEAR_HTTP_FLAG(phttp, HRF_PARSE_DONE);
	...
	if (!cl7_proxy && (nsconf != NULL)) {
		...
		if (CHECK_HTTP_FLAG(phttp, HRF_HAS_QUERY) &&
		    ...
		    !(pe_action & PE_ACTION_REQ_CACHE_OBJECT) &&
		     (!CHECK_HTTP_FLAG(phttp, HRF_TIER2_MODE))
		   ) {
		...
		if (!ret && (dtype == DT_CACHECONTROL_ENUM) ) {
			if (((ckdp->u.dt_cachecontrol_enum.mask & CT_MASK_NO_CACHE) || 
				...
				if ((oss_method == OSS_HTTP_NODE_MAP) &&
				    (!CHECK_HTTP_FLAG(phttp, HRF_METHOD_HEAD)) &&
                                    (!nsconf->http_config->origin.u.http.include_orig_ip_port)) {
	...
	if ((nsconf->http_config->policies.cache_ingest.incapable_byte_ran_origin == NKN_TRUE) &&
		((nsconf->proxy_mode == PROXY_TRANSPARENT) || (nsconf->proxy_mode == PROXY_VIRTUAL_DOMAIN))) {
		    phttp->tunnel_reason_code = NKN_TR_REQ_INCAPABLE_BR_ORIGIN;
		    glob_tunnel_req_incapable_byte_range_origin++;
		    goto limit_fail;
	} 
...
{
     return http_parse_uri_internal(phttp, p, 0) ;
}

int http_parse_uri_internal(http_cb_t * phttp, const char * p, int pe_url_rewrite)
{
        const char * ps;
	const char * p_query = 0;
	int len;
	...
	if (ps && *ps && (len > 0) && 
		!((CHECK_HTTP_FLAG(phttp, HRF_METHOD_OPTIONS) ||
		 CHECK_HTTP_FLAG(phttp, HRF_METHOD_TRACE) )&& *ps == '*'))  {
                ...
                if (pe_url_rewrite) {
		    add_known_header(hdr, MIME_HDR_X_NKN_REMAPPED_URI, ps, len);
                }
		add_known_header(hdr, MIME_HDR_X_NKN_URI, ps, len);
	} else if ((ps && len == 0) || 
			(ps && *ps == '*' && len == 1)){
                ...
                if (pe_url_rewrite) {
		    add_known_header(hdr, MIME_HDR_X_NKN_REMAPPED_URI, "/", sizeof("/") - 1);
                }
