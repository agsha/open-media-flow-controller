...
am_update_hotness(nkn_hot_t *in_hotness, int num_hits, time_t object_ts,
		    uint64_t hotness_inc, char *uri __attribute((unused)))
{
    ...
    if(!object_ts)
	object_ts = (AO_load(&nkn_cur_ts)/(DELAY_GRANULARITY));
	...
	if(((AO_load(&nkn_cur_ts)/(DELAY_GRANULARITY)) - nkn_am_hotness_prev_time_window) >
		nkn_am_hotness_time_window) {
	    nkn_am_hotness_prev_time_window = (AO_load(&nkn_cur_ts)/(DELAY_GRANULARITY));
	}
    } else
	nkn_am_hotness_prev_time_window = (AO_load(&nkn_cur_ts)/(DELAY_GRANULARITY));
...
#if 0
	DBG_LOG(MSG3, MOD_AM, "Hotness update for uri %s hit %d objectts %lld"
		" cur_interval %lld interval_check_start %lld cur_time %lld",
	...
	DBG_LOG(MSG3, MOD_AM, "Hotness update for uri %s hit %d"
		" objectts %lld cur_interval %lld interval_check_start %lld"
		" cur_time %lld total_int %lld glob_interval %lld"
		" total_hits %lld hotness %d", uri,
