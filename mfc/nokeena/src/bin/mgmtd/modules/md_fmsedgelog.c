...
int md_fmsedgelog_init(const lc_dso_info *info, void *data);

static md_upgrade_rule_array *md_fmsedgelog_ug_rules = NULL;
/*----------------------------------------------------------------------------
 * PRIVATE FUNCTION PROTOTYPES
 *---------------------------------------------------------------------------*/
static int md_fmsedgelog_file_upload_config(md_commit *commit, mdb_db **db,
 ...
 *---------------------------------------------------------------------------*/
int
md_fmsedgelog_init(const lc_dso_info *info, void *data)
{
    int err = 0;
     ...
     */
    err = mdm_add_module(
                    "fmsedgelog",                           // module_name
                    1,                                  // module_version
                    "/nkn/fmsedgelog",                      // root_node_name
                    ...
                    &md_fmsedgelog_ug_rules,              // updown_arg
                    NULL,                               // initial_func
                    NULL,                               // initial_arg
                    NULL,                               // mon_get_func
                    NULL,                               // mon_get_arg
    ...
    /*
     * Setup config node - /nkn/fmsedgelog/config/path
     *  - Configure log path
     *  - Default is "/var/log/nkn"
     */
    err = mdm_new_node(module, &node);
    bail_error(err);
    node->mrn_name              = "/nkn/fmsedgelog/config/path";
    ...
    node->mrn_description       = "Path where fmsedge log is saved";
    ...
    err = mdm_new_node(module, &node);
    bail_error(err);
    node->mrn_name              = "/nkn/fmsedgelog/config/syslog";
    ...
    err = mdm_new_node(module, &node);
    bail_error(err);
    node->mrn_name              = "/nkn/fmsedgelog/config/enable";
    node->mrn_value_type        = bt_bool;
    node->mrn_initial_value     = "true";
    node->mrn_node_reg_flags    = mrf_flags_reg_config_literal;
    node->mrn_cap_mask          = mcf_cap_node_restricted;
    node->mrn_description       = "Enable fmsedgelog";
    err = mdm_add_node(module, &node);
    bail_error(err);
    ...
    /*
     * Setup config node - /nkn/fmsedgelog/config/filename
     *  - Configure FileName
     *  - Default is "fmsedge.log"
     */
    err = mdm_new_node(module, &node);
    bail_error(err);
    node->mrn_name              = "/nkn/fmsedgelog/config/filename";
    node->mrn_value_type        = bt_string;
    node->mrn_limit_str_max_chars = 256;
    node->mrn_initial_value     = "edge.00.log";
    node->mrn_node_reg_flags    = mrf_flags_reg_config_literal;
    node->mrn_limit_str_no_charlist = "/\\*:|`\"?&^!@#%$(){}[]<>,";
    node->mrn_cap_mask          = mcf_cap_node_restricted;
    node->mrn_description       = "Filename where FMS Edge log is saved";
    err = mdm_add_node(module, &node);
    bail_error(err);

    /*
     * Setup config node - /nkn/fmsedgelog/config/max-file-size
    ...
    err = mdm_new_node(module, &node);
    bail_error(err);
    node->mrn_name              = "/nkn/fmsedgelog/config/max-filesize";
    ...
    node->mrn_description       = "Maximum single filesize allocated "
                                  "for one fmsedgelog file, in MiB";
    err = mdm_add_node(module, &node);
    bail_error(err);

    err = mdm_new_node(module, &node);
    bail_error(err);
    node->mrn_name              = "/nkn/fmsedgelog/config/time-interval";
    ...
    err = mdm_new_node(module, &node);
    bail_error(err);
    node->mrn_name              = "/nkn/fmsedgelog/config/upload/url";
    node->mrn_value_type        = bt_string;
    ...
    err = mdm_new_node(module, &node);
    bail_error(err);
    node->mrn_name              = "/nkn/fmsedgelog/config/upload/pass";
    node->mrn_value_type        = bt_string;
    node->mrn_initial_value     = "";
    node->mrn_node_reg_flags    = mrf_flags_reg_config_literal;
    node->mrn_cap_mask          = mcf_cap_node_restricted;
    ...
    bail_error(err);
    node->mrn_name              = "/nkn/fmsedgelog/action/upload";
    node->mrn_node_reg_flags    = mrf_flags_reg_action;
    node->mrn_cap_mask          = mcf_cap_action_privileged;
    node->mrn_action_config_func = md_fmsedgelog_file_upload_config;
    ...
    err = mdm_new_node(module, &node);
    bail_error(err);
    node->mrn_name              = "/nkn/fmsedgelog/config/on-the-hour";
    node->mrn_value_type        = bt_bool;
    node->mrn_initial_value     = "false";
    node->mrn_node_reg_flags    = mrf_flags_reg_config_literal;
    node->mrn_cap_mask          = mcf_cap_node_restricted;
...
static int
md_fmsedgelog_file_upload_config(md_commit *commit, mdb_db **db,
	const char *action_name, bn_binding_array *params,
	void *cb_arg)
{
    int err = 0;
    ...
    bail_require(!strcmp(action_name, "/nkn/fmsedgelog/action/upload"));
    ...
    err = bn_binding_new_str(&binding, "/nkn/fmsedgelog/config/upload/url",
    ...
    err = bn_binding_new_str(&binding, "/nkn/fmsedgelog/config/upload/pass",
