...
int md_fuselog_init(const lc_dso_info *info, void *data);

static md_upgrade_rule_array *md_fuselog_ug_rules = NULL;
/*----------------------------------------------------------------------------
 * PRIVATE FUNCTION PROTOTYPES
 *---------------------------------------------------------------------------*/
static int
md_fuselog_commit_check(
        md_commit *commit,
        const mdb_db *old_db,
        const mdb_db *new_db,
...
static int md_fuselog_file_upload_config(md_commit *commit, mdb_db **db,
                    const char *action_name, bn_binding_array *params,
                    void *cb_arg);

static int
md_fuselog_files_iterate(md_commit *commit, const mdb_db *db,
...
static int
md_fuselog_files_get(md_commit *commit, const mdb_db *db,
 ...
 *---------------------------------------------------------------------------*/
int
md_fuselog_init(const lc_dso_info *info, void *data)
{
    int err = 0;
    ...
    err = mdm_add_module(
                    ...
                    &md_fuselog_ug_rules,              // updown_arg
                    NULL,                               // initial_func
                    NULL,                               // initial_arg
                    NULL,                               // mon_get_func
                    NULL,                               // mon_get_arg
    ...
    /*
     * Setup config node - /nkn/fuselog/config/path
     *  - Configure log path
     *  - Default is "/var/log/nkn"
     */
    err = mdm_new_node(module, &node);
    bail_error(err);
    node->mrn_name              = "/nkn/fuselog/config/path";
    ...
    node->mrn_description       = "Path where fuse log is saved";
    ...
    err = mdm_new_node(module, &node);
    bail_error(err);
    node->mrn_name              = "/nkn/fuselog/config/syslog";
    ...
    err = mdm_new_node(module, &node);
    bail_error(err);
    node->mrn_name              = "/nkn/fuselog/config/enable";
    node->mrn_value_type        = bt_bool;
    node->mrn_initial_value     = "true";
    node->mrn_node_reg_flags    = mrf_flags_reg_config_literal;
    node->mrn_cap_mask          = mcf_cap_node_restricted;
    node->mrn_description       = "Enable fuselog";
    err = mdm_add_node(module, &node);
    bail_error(err);
    ...
    /*
     * Setup config node - /nkn/fuselog/config/filename
     *  - Configure FileName
     *  - Default is "fuse.log"
     */
    err = mdm_new_node(module, &node);
    bail_error(err);
    node->mrn_name              = "/nkn/fuselog/config/filename";
    node->mrn_value_type        = bt_string;
    node->mrn_limit_str_max_chars = 256;
    node->mrn_initial_value     = "fuse.log";
    node->mrn_node_reg_flags    = mrf_flags_reg_config_literal;
    node->mrn_limit_str_no_charlist = "/\\*:|`\"?&^!@#%$(){}[]<>,";
    node->mrn_cap_mask          = mcf_cap_node_restricted;
    node->mrn_description       = "Filename where fuse log is saved";
    err = mdm_add_node(module, &node);
    bail_error(err);

    /*
     * Setup config node - /nkn/fuselog/config/max-file-size
    ...
    err = mdm_new_node(module, &node);
    bail_error(err);
    node->mrn_name              = "/nkn/fuselog/config/max-filesize";
    ...
    node->mrn_description       = "Maximum single filesize allocated "
                                  "for one fuselog file, in MiB";
    err = mdm_add_node(module, &node);
    bail_error(err);

    err = mdm_new_node(module, &node);
    bail_error(err);
    node->mrn_name              = "/nkn/fuselog/config/time-interval";
    ...
    err = mdm_new_node(module, &node);
    bail_error(err);
    node->mrn_name              = "/nkn/fuselog/config/upload/url";
    node->mrn_value_type        = bt_string;
    ...
    err = mdm_new_node(module, &node);
    bail_error(err);
    node->mrn_name              = "/nkn/fuselog/config/upload/pass";
    node->mrn_value_type        = bt_string;
    node->mrn_initial_value     = "";
    node->mrn_node_reg_flags    = mrf_flags_reg_config_literal;
    node->mrn_cap_mask          = mcf_cap_node_restricted;
    ...
    bail_error(err);
    node->mrn_name              = "/nkn/fuselog/action/upload";
    node->mrn_node_reg_flags    = mrf_flags_reg_action;
    node->mrn_cap_mask          = mcf_cap_action_privileged;
    node->mrn_action_config_func = md_fuselog_file_upload_config;
    ...
    err = mdm_new_node(module, &node);
    bail_error(err);
    node->mrn_name              = "/nkn/fuselog/config/on-the-hour";
    node->mrn_value_type        = bt_bool;
    node->mrn_initial_value     = "false";
    node->mrn_node_reg_flags    = mrf_flags_reg_config_literal;
    node->mrn_cap_mask          = mcf_cap_node_restricted;
    ...
    err = mdm_new_node(module, &node);
    bail_error(err);
    node->mrn_name		= "/nkn/fuselog/logfiles/*";
    node->mrn_value_type	= bt_string;
    node->mrn_node_reg_flags	= mrf_flags_reg_monitor_wildcard;
    node->mrn_cap_mask		= mcf_cap_node_restricted;
    node->mrn_mon_get_func	= md_fuselog_files_get;
    node->mrn_mon_iterate_func	= md_fuselog_files_iterate;
    node->mrn_description	= "List of archieved FMS connectorlog files available";
    err = mdm_add_node(module, &node);
    bail_error(err);



    err = md_upgrade_rule_array_new(&md_fuselog_ug_rules);
    bail_error(err);
    ra = md_fuselog_ug_rules;

    MD_NEW_RULE(ra, 1, 2);
    rule->mur_upgrade_type =    MUTT_ADD;
    rule->mur_name =            "/nkn/fuselog/config/on-the-hour";
        ...
        void *arg)
{
	...
	if(change->mdc_change_type == mdct_modify) {
	    if (bn_binding_name_parts_pattern_match_va(change->mdc_name_parts, 0, 4, "nkn", "fuselog", "config", "time-interval"))
	    {
                    ...
                    void *cb_arg)
{
    ...
    bail_require(!strcmp(action_name, "/nkn/fuselog/action/upload"));
    ...
    err = bn_binding_new_str(&binding, "/nkn/fuselog/config/upload/url",
    ...
    err = bn_binding_new_str(&binding, "/nkn/fuselog/config/upload/pass",
