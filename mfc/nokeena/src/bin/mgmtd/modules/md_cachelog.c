...
int md_cachelog_init(const lc_dso_info *info, void *data);

static md_upgrade_rule_array *md_cachelog_ug_rules = NULL;

/*----------------------------------------------------------------------------
 * PRIVATE FUNCTION PROTOTYPES
 *---------------------------------------------------------------------------*/
static int
md_cachelog_commit_check(
        md_commit *commit,
        const mdb_db *old_db,
        const mdb_db *new_db,
...
static int
md_cachelog_commit_side_effects(
        md_commit *commit,
        const mdb_db *old_db,
        mdb_db *inout_db,
        mdb_db_change_array *change_list,
...
static int md_cachelog_file_upload_config(md_commit *commit, mdb_db **db,
                    const char *action_name, bn_binding_array *params,
                    void *cb_arg);

static int
md_cachelog_files_iterate(md_commit *commit, const mdb_db *db,
...
static int
md_cachelog_files_get(md_commit *commit, const mdb_db *db,
 ...
 *---------------------------------------------------------------------------*/
int
md_cachelog_init(const lc_dso_info *info, void *data)
{
    int err = 0;
     ...
     */
    err = mdm_add_module(
                    "cachelog",                           // module_name
                    3,                                  // module_version
                    "/nkn/cachelog",                      // root_node_name
                    NULL,                               // root_config_name
                    0,				       // md_mod_flags
                    md_cachelog_commit_side_effects,      // side_effects_func
                    NULL,                               // side_effects_arg
                    md_cachelog_commit_check,             // check_func
                    NULL,                               // check_arg
                    md_cachelog_commit_apply,             // apply_func
                    NULL,                               // apply_arg
                    200,                                // commit_order
                    0,                                  // apply_order
                    md_generic_upgrade_downgrade,        // updown_func
                    &md_cachelog_ug_rules,              // updown_arg
                    NULL,                               // initial_func
                    NULL,                               // initial_arg
                    NULL,                               // mon_get_func
                    NULL,                               // mon_get_arg
    ...
    /*
     * Setup config node - /nkn/cachelog/config/path
     *  - Configure log path
     *  - Default is "/var/log/nkn"
     */
    err = mdm_new_node(module, &node);
    bail_error(err);
    node->mrn_name              = "/nkn/cachelog/config/path";
    ...
    node->mrn_description       = "Path where cache log is saved";
    ...
    err = mdm_new_node(module, &node);
    bail_error(err);
    node->mrn_name              = "/nkn/cachelog/config/syslog";
    ...
    err = mdm_new_node(module, &node);
    bail_error(err);
    node->mrn_name              = "/nkn/cachelog/config/enable";
    node->mrn_value_type        = bt_bool;
    node->mrn_initial_value     = "true";
    node->mrn_node_reg_flags    = mrf_flags_reg_config_literal;
    node->mrn_cap_mask          = mcf_cap_node_restricted;
    node->mrn_description       = "Enable cachelog";
    err = mdm_add_node(module, &node);
    bail_error(err);
    ...
    /*
     * Setup config node - /nkn/cachelog/config/filename
     *  - Configure FileName
     *  - Default is "cache.log"
     */
    err = mdm_new_node(module, &node);
    bail_error(err);
    node->mrn_name              = "/nkn/cachelog/config/filename";
    node->mrn_value_type        = bt_string;
    node->mrn_limit_str_max_chars = 256;
    node->mrn_initial_value     = "cache.log";
    node->mrn_node_reg_flags    = mrf_flags_reg_config_literal;
    node->mrn_limit_str_no_charlist = "/\\*:|`\"?&^!@#%$(){}[]<>,";
    node->mrn_cap_mask          = mcf_cap_node_restricted;
    node->mrn_description       = "Filename where cache log is saved";
    err = mdm_add_node(module, &node);
    bail_error(err);

    /*
     * Setup config node - /nkn/cachelog/config/max-file-size
    ...
    err = mdm_new_node(module, &node);
    bail_error(err);
    node->mrn_name              = "/nkn/cachelog/config/max-filesize";
    ...
    node->mrn_description       = "Maximum single filesize allocated "
                                  "for one cachelog file, in MiB";
    err = mdm_add_node(module, &node);
    bail_error(err);

    err = mdm_new_node(module, &node);
    bail_error(err);
    node->mrn_name              = "/nkn/cachelog/config/time-interval";
    ...
    err = mdm_new_node(module, &node);
    bail_error(err);
    node->mrn_name              = "/nkn/cachelog/config/upload/url";
    node->mrn_value_type        = bt_string;
    ...
    err = mdm_new_node(module, &node);
    bail_error(err);
    node->mrn_name              = "/nkn/cachelog/config/upload/pass";
    node->mrn_value_type        = bt_string;
    node->mrn_initial_value     = "";
    node->mrn_node_reg_flags    = mrf_flags_reg_config_literal;
    node->mrn_cap_mask          = mcf_cap_node_restricted;
    ...
    bail_error(err);
    node->mrn_name              = "/nkn/cachelog/action/upload";
    node->mrn_node_reg_flags    = mrf_flags_reg_action;
    node->mrn_cap_mask          = mcf_cap_action_privileged;
    node->mrn_action_config_func = md_cachelog_file_upload_config;
    ...
    err = mdm_new_node(module, &node);
    bail_error(err);
    node->mrn_name              = "/nkn/cachelog/config/on-the-hour";
    node->mrn_value_type        = bt_bool;
    node->mrn_initial_value     = "false";
    node->mrn_node_reg_flags    = mrf_flags_reg_config_literal;
    node->mrn_cap_mask          = mcf_cap_node_restricted;
    ...
    err = mdm_new_node(module, &node);
    bail_error(err);
    node->mrn_name              = "/nkn/cachelog/config/rotate";
    node->mrn_value_type        = bt_bool;
    node->mrn_initial_value     = "true";
    node->mrn_node_reg_flags    = mrf_flags_reg_config_literal;
    node->mrn_cap_mask          = mcf_cap_node_restricted;
    node->mrn_description       = "Enable the cachelog log rotation";
    err = mdm_add_node(module, &node);
    bail_error(err);
    ...
    err = mdm_new_node(module, &node);
    bail_error(err);
    node->mrn_name		= "/nkn/cachelog/logfiles/*";
    node->mrn_value_type	= bt_string;
    node->mrn_node_reg_flags	= mrf_flags_reg_monitor_wildcard;
    node->mrn_cap_mask		= mcf_cap_node_restricted;
    node->mrn_mon_get_func	= md_cachelog_files_get;
    node->mrn_mon_iterate_func	= md_cachelog_files_iterate;
    node->mrn_description	= "List of archieved cachelog files available";
    err = mdm_add_node(module, &node);
    bail_error(err);


    err = md_upgrade_rule_array_new(&md_cachelog_ug_rules);
    bail_error(err);
    ra = md_cachelog_ug_rules;

    MD_NEW_RULE(ra, 1, 2);
    rule->mur_upgrade_type =    MUTT_ADD;
    rule->mur_name =            "/nkn/cachelog/config/on-the-hour";
    ...
    MD_NEW_RULE(ra, 2, 3);
    rule->mur_upgrade_type =    MUTT_ADD;
    rule->mur_name =            "/nkn/cachelog/config/rotate";
    rule->mur_new_value_type =  bt_bool;
        ...
        void *arg)
{
	...
	uint32 new_time_interval = 0;
	    ...
	    if(change->mdc_change_type == mdct_modify) {
                if (bn_binding_name_parts_pattern_match_va(change->mdc_name_parts, 0, 4, "nkn", "cachelog", "config", "time-interval"))
                {
                    ...
                    void *cb_arg)
{
    ...
    bail_require(!strcmp(action_name, "/nkn/cachelog/action/upload"));
    ...
    err = bn_binding_new_str(&binding, "/nkn/cachelog/config/upload/url",
    ...
    err = bn_binding_new_str(&binding, "/nkn/cachelog/config/upload/pass",
                     ...
                     void *arg)
{
    ...
    /* Verify this is "/nkn/cachelog/logfiles/ *" */
