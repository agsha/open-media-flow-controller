...
static const char rcsid[] = "$Id: web_logging.c,v 1.30 2011/05/17 00:54:08 et Exp $";
...
static const char wlog_get_log_lines_cmd_name[] = "tms::get-log-lines";
static const char wlog_get_filtered_log_lines_cmd_name[] =
    "tms::get-filtered-log-lines";
static const char wlog_get_log_line_count_cmd_name[] =
    "tms::get-log-line-count";
static const char wlog_get_filtered_log_line_count_cmd_name[] =
    "tms::get-filtered-log-line-count";
static const char wlog_continuous_log_cmd_name[] = "tms::continuous-log";
static const uint32 wlog_log_buf_size = 16384;
static const char wlog_force_rotate_action[] = "rotate";
static const char wlog_download_action[] = "download-log";
static const char wlog_download_prefix[] = "/var/log";

/*-----------------------------------------------------------------------------
 * PROTOTYPES
 */

static int wlog_open_filtered_log(const char *filter, int *ret_fd);
static int wlog_open_archived_log(const char *file_num, int *ret_fd);
static int wlog_open_filtered_archived_log(const char *filter,
                                           const char *file_num, int *ret_fd);

static int wlog_get_log_lines(Tcl_Interp *interpreter,
                              int fd,
                              uint32 start,
                              uint32 end,
                              Tcl_Obj **ret_list);

static int wlog_get_log_lines_cmd(ClientData clientData,
                                  Tcl_Interp *interpreter,
                                  int objc,
                                  Tcl_Obj *CONST objv[]);

static int wlog_continuous_log_cmd(ClientData clientData,
                                   Tcl_Interp *interpreter,
                                   int objc,
                                   Tcl_Obj *CONST objv[]);

static int wlog_get_log_line_count(int fd, uint32 *ret_lines);

static int wlog_get_log_line_count_cmd(ClientData clientData,
                                       Tcl_Interp *interpreter,
                                       int objc,
                                       Tcl_Obj *CONST objv[]);

static web_action_result wlog_force_rotate_action_handler(web_context *ctx,
                                                          void *param);

static web_action_result wlog_download_action_handler(web_context *ctx,
                                                      void *param);

int web_logging_init(const lc_dso_info *info, void *data);
...
static int
wlog_open_filtered_log(const char *filter, int *ret_fd)
{
...
static int
wlog_open_archived_log(const char *file_num, int *ret_fd)
{
    lc_launch_params *launch_params = NULL;
    lc_launch_result result = LC_LAUNCH_RESULT_INIT;
    char *file_name = NULL;
...
static int
wlog_open_filtered_archived_log(const char *filter, const char *file_num,
                                int *ret_fd)
{
    lc_launch_params *launch_params = NULL;
    lc_launch_result result = LC_LAUNCH_RESULT_INIT;
                   ...
                   uint32 start, uint32 end, Tcl_Obj **ret_list)
{
    ...
    do {
        ...
        errno = 0;
        count = lf_read_bytes(fd, buf, wlog_log_buf_size);
        ...
        for (i = 0; i < (uint32)count; ++i) {
            if (buf[i] == '\n') {
                if (cur_line + 1 >= start && cur_line + 1 < end) {
                    ...
                    ts_free(&line_buf_esc);
                    err = web_escape_entities(ts_str(line_buf), &line_buf_esc);
                    bail_error(err);

                    line_obj = Tcl_NewStringObj(ts_str(line_buf_esc),
                                                ts_length(line_buf_esc));
                    bail_null(line_obj);
...
static int
wlog_get_log_lines_cmd(ClientData clientData,
                       Tcl_Interp *interpreter,
                       int objc,
                       Tcl_Obj *CONST objv[])
{
    ...
    if (objc == 3) {
        ...
        err = wlog_open_archived_log(file_num, &fd);
        bail_error(err);
    }

    err = wlog_get_log_lines(interpreter, fd, start_line, end_line, &list);
    bail_error(err);
...
static int
wlog_get_filtered_log_lines_cmd(ClientData clientData,
                                Tcl_Interp *interpreter,
                                int objc,
                                Tcl_Obj *CONST objv[])
{
    ...
    if (objc == 4) {
        err = wlog_open_filtered_log(filter, &fd);
        bail_error(err);
    } else {
        file_num = Tcl_GetString(objv[4]);
        bail_null(file_num);

        err = wlog_open_filtered_archived_log(filter, file_num, &fd);
        bail_error(err);
    }
...
static int
wlog_get_log_line_count(int fd, uint32 *ret_lines)
{
    char buf[wlog_log_buf_size];
    uint32 lines = 0;
    int count = 0;
...
static int
wlog_get_log_line_count_cmd(ClientData clientData,
                            Tcl_Interp *interpreter,
                            int objc,
                            Tcl_Obj *CONST objv[])
{
    ...
    if (fd >= 0) {
        err = wlog_get_log_line_count(fd, &lines);
        bail_error(err);
    }
...
static int
wlog_get_filtered_log_line_count_cmd(ClientData clientData,
                                     Tcl_Interp *interpreter,
                                     int objc,
                                     Tcl_Obj *CONST objv[])
{
...
static int
wlog_continuous_log_cmd(ClientData clientData,
                        Tcl_Interp *interpreter,
                        int objc,
                        Tcl_Obj *CONST objv[])
{
    ...
    do {
        errno = 0;
        count = lf_read_bytes(fd, buf, wlog_log_buf_size - 1);
        if (count == -1 && errno == EINTR) { continue; }
        bail_require_errno(count >= 0, I_("Reading log file"));
...
wlog_force_rotate_action_handler(web_context *ctx, void *param)
{
    int err = 0;
    tstring *msg = NULL;
    uint32 code = 0;
    web_action_result result = 0;

    if (web_request_param_exists_str(g_web_data, "rotate_logs",
                                     ps_post_data)) {
        ...
        if (code || (msg && ts_num_chars(msg) > 0)) {
            ...
            err = web_set_msg_result_str(g_web_data, 0, 
                                         _("Log rotation successful."));
            bail_error(err);
        }

        /*
         * If we didn't do this, changes they made would still be left
         * on the form, and since they just submitted the form, they
         * might think the changes had been applied.
         */
        err = web_clear_post_data(g_web_data);
        bail_error(err);
    }
    else {
        result = web_run_action(ctx, wcf_config_form_action_name);
        bail_require(result != war_error);
    }
...
static web_action_result
wlog_download_action_handler(web_context *ctx, void *param)
{
    int err = 0;
    ...
    // Juniper change: use "f_file", ps_post_data  instead of "file", ps_query_string
    // Not sure why this change was made.
  //err = web_get_request_param_str(g_web_data, "file", ps_query_string,
  //                                &file);
    ...
    err = lf_path_from_dir_file(wlog_download_prefix, ts_str(file),
                                &file_path);
    ...
    if (archived) {
        ...
        // Juniper change: call web_send_raw_file_with_fileinfo() instead of
        //                      web_send_raw_process_output_str()
        // Not sure why this change was made.
...
int
web_logging_init(const lc_dso_info *info, void *data)
{
    ...
    cmd = Tcl_CreateObjCommand(
        ctx->wctx_interpreter, (char *)wlog_get_log_lines_cmd_name,
        wlog_get_log_lines_cmd, NULL, NULL);
    bail_null(cmd);

    cmd = Tcl_CreateObjCommand(
        ctx->wctx_interpreter, (char *)wlog_get_filtered_log_lines_cmd_name,
        wlog_get_filtered_log_lines_cmd, NULL, NULL);
    bail_null(cmd);

    cmd = Tcl_CreateObjCommand(
        ctx->wctx_interpreter, (char *)wlog_get_log_line_count_cmd_name,
        wlog_get_log_line_count_cmd, NULL, NULL);
    bail_null(cmd);

    cmd = Tcl_CreateObjCommand(
        ctx->wctx_interpreter,
        (char *)wlog_get_filtered_log_line_count_cmd_name,
        wlog_get_filtered_log_line_count_cmd, NULL, NULL);
    bail_null(cmd);

    cmd = Tcl_CreateObjCommand(
        ctx->wctx_interpreter, (char *)wlog_continuous_log_cmd_name,
        wlog_continuous_log_cmd, NULL, NULL);
    bail_null(cmd);

    err = web_register_action_str(g_rh_data, wlog_force_rotate_action, NULL,
                                  wlog_force_rotate_action_handler);
    bail_error(err);

    err = web_register_action_str(g_rh_data, wlog_download_action, NULL,
                                  wlog_download_action_handler);
    bail_error(err);
