...
static const char wlog_nkn_get_log_lines_cmd_name[] = "tms::get-log-lines";
static const char wlog_nkn_get_filtered_log_lines_cmd_name[] =
    "tms::get-filtered-log-lines";
static const char wlog_nkn_get_log_line_count_cmd_name[] =
    "tms::get-log-line-count";
static const char wlog_nkn_get_filtered_log_line_count_cmd_name[] =
    "tms::get-filtered-log-line-count";
static const char wlog_nkn_continuous_log_cmd_name[] = "tms::continuous-log";
static const uint32 wlog_log_buf_size = 16384;
static const char wlog_nkn_force_rotate_action[] = "rotate";
static const char wcf_nkn_download_action[] = "download-nknlog";
static const char wlog_nkn_download_prefix[] = "/var/log/nkn";

/*-----------------------------------------------------------------------------
 * PROTOTYPES
 */

static int wlog_nkn_open_filtered_log(const char *filter, int *ret_fd);
static int wlog_nkn_open_archived_log(const char *file_num, int *ret_fd);
static int wlog_nkn_open_filtered_archived_log(const char *filter,
                                           const char *file_num, int *ret_fd);

static int wlog_nkn_get_log_lines(Tcl_Interp *interpreter,
                              int fd,
                              uint32 start,
                              uint32 end,
                              Tcl_Obj **ret_list);

static int wlog_nkn_get_log_lines_cmd(ClientData clientData,
                                  Tcl_Interp *interpreter,
                                  int objc,
                                  Tcl_Obj *CONST objv[]);

static int wlog_nkn_continuous_log_cmd(ClientData clientData,
                                   Tcl_Interp *interpreter,
                                   int objc,
                                   Tcl_Obj *CONST objv[]);

static int wlog_nkn_get_log_line_count(int fd, uint32 *ret_lines);

static int wlog_nkn_get_log_line_count_cmd(ClientData clientData,
                                       Tcl_Interp *interpreter,
                                       int objc,
                                       Tcl_Obj *CONST objv[]);

static web_action_result wlog_nkn_force_rotate_action_handler(web_context *ctx,
                                                          void *param);

static web_action_result wlog_nkn_download_action_handler(web_context *ctx,
                                                      void *param);

int web_nkn_logging_init(const lc_dso_info *info, void *data);
...
static int
wlog_nkn_open_filtered_log(const char *filter, int *ret_fd)
{
...
static int
wlog_nkn_open_archived_log(const char *file_num, int *ret_fd)
{
    lc_launch_params *launch_params = NULL;
    lc_launch_result result;
    char *file_name = NULL;
...
static int
wlog_nkn_open_filtered_archived_log(const char *filter, const char *file_num,
                                int *ret_fd)
{
    lc_launch_params *launch_params = NULL;
    lc_launch_result result;
...
static int
wlog_nkn_get_log_lines(Tcl_Interp *interpreter, int fd,
...
static int
wlog_nkn_get_log_lines_cmd(ClientData clientData,
                       Tcl_Interp *interpreter,
                       int objc,
                       Tcl_Obj *CONST objv[])
{
    ...
    if (objc == 3) {
        ...
        err = wlog_nkn_open_archived_log(file_num, &fd);
        bail_error(err);
    }

    err = wlog_nkn_get_log_lines(interpreter, fd, start_line, end_line, &list);
    bail_error(err);
...
static int
wlog_nkn_get_filtered_log_lines_cmd(ClientData clientData,
                                Tcl_Interp *interpreter,
                                int objc,
                                Tcl_Obj *CONST objv[])
{
    ...
    if (objc == 4) {
        err = wlog_nkn_open_filtered_log(filter, &fd);
        bail_error(err);
    } else {
        file_num = Tcl_GetString(objv[4]);
        bail_null(file_num);

        err = wlog_nkn_open_filtered_archived_log(filter, file_num, &fd);
        bail_error(err);
    }
...
static int
wlog_nkn_get_log_line_count(int fd, uint32 *ret_lines)
{
    char buf[wlog_log_buf_size];
    uint32 lines = 0;
    int count = 0;
...
static int
wlog_nkn_get_log_line_count_cmd(ClientData clientData,
                            Tcl_Interp *interpreter,
                            int objc,
                            Tcl_Obj *CONST objv[])
{
    ...
    if (fd >= 0) {
        err = wlog_nkn_get_log_line_count(fd, &lines);
        bail_error(err);
    }
...
static int
wlog_nkn_get_filtered_log_line_count_cmd(ClientData clientData,
                                     Tcl_Interp *interpreter,
                                     int objc,
                                     Tcl_Obj *CONST objv[])
{
...
static int
wlog_nkn_continuous_log_cmd(ClientData clientData,
                        Tcl_Interp *interpreter,
                        int objc,
                        Tcl_Obj *CONST objv[])
{
...
wlog_nkn_force_rotate_action_handler(web_context *ctx, void *param)
{
    tstring *msg = NULL;
    uint32 code = 0;
    int err = 0;
...
static web_action_result
wlog_nkn_download_action_handler(web_context *ctx, void *param)
{
    int err = 0;
    ...
    err = lf_path_from_dir_file(wlog_nkn_download_prefix, ts_str(file),
                                &file_path);
...
int
web_nkn_logging_init(const lc_dso_info *info, void *data)
{
    ...
    cmd = Tcl_CreateObjCommand(
        ctx->wctx_interpreter, (char *)wlog_nkn_get_log_lines_cmd_name,
        wlog_nkn_get_log_lines_cmd, NULL, NULL);
    bail_null(cmd);

    cmd = Tcl_CreateObjCommand(
        ctx->wctx_interpreter, (char *)wlog_nkn_get_filtered_log_lines_cmd_name,
        wlog_nkn_get_filtered_log_lines_cmd, NULL, NULL);
    bail_null(cmd);

    cmd = Tcl_CreateObjCommand(
        ctx->wctx_interpreter, (char *)wlog_nkn_get_log_line_count_cmd_name,
        wlog_nkn_get_log_line_count_cmd, NULL, NULL);
    bail_null(cmd);

    cmd = Tcl_CreateObjCommand(
        ctx->wctx_interpreter,
        (char *)wlog_nkn_get_filtered_log_line_count_cmd_name,
        wlog_nkn_get_filtered_log_line_count_cmd, NULL, NULL);
    bail_null(cmd);

    cmd = Tcl_CreateObjCommand(
        ctx->wctx_interpreter, (char *)wlog_nkn_continuous_log_cmd_name,
        wlog_nkn_continuous_log_cmd, NULL, NULL);
    bail_null(cmd);

    err = web_register_action_str(g_rh_data, wlog_nkn_force_rotate_action, NULL,
                                  wlog_nkn_force_rotate_action_handler);
    bail_error(err);

    err = web_register_action_str(g_rh_data, wcf_nkn_download_action, NULL,
                                  wlog_nkn_download_action_handler);
    bail_error(err);
