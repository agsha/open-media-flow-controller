/*
 *
 * Filename:  Web_config_form_ntp.c
...
static const char wcf_action_name[] = "config-form-ntp";
...
web_action_result
wcf_config_form_ntp_handler(web_context *ctx, void *param);

int web_config_form_ntp_init(const lc_dso_info *info, void *data);
...
wcf_handle_add(void)
{
    ...
    lc_log_basic(LOG_NOTICE, "Atleast shall i see this");
...
//  node_name = smprintf("%s/", ts_str(root));
//  bail_null(node_name);

//  lc_log_basic(LOG_NOTICE, "%s", node_name);

    /* Convert the hostname into ip address since the NTP node checks for ip address */
    struct hostent *hent;
    lc_log_basic(LOG_NOTICE, "can i see you");    
    lc_log_basic(LOG_NOTICE, "%s", windex_value_esc);
    // Porting to FIR - use new API to check for IPv4
    if (!lia_str_is_ipv4addr_quick(windex_value_esc)) {
	lc_log_basic(LOG_NOTICE, "this is not an ip address but hostname");
        hent = gethostbyname(windex_value_esc);
        if (NULL == hent) {
	    lc_log_basic(LOG_NOTICE, "inside hent is NULL");
	    err = ts_new_sprintf(&msg, _("Not able to resolve the hostname - %s" ),
                                     windex_value_esc);
            code = 1;
            bail_error(err);
	    err = web_set_msg_result(g_web_data, code, msg);
	    bail_error(err);
	    lc_log_basic(LOG_NOTICE, "after the web message");
            goto bail;
        }

        if (hent->h_addr_list[0] != NULL) {
	    lc_log_basic(LOG_NOTICE, "inside hent address list  is not NULL");
            ts_new_or_replace_str(&windex_value,inet_ntoa(*(struct in_addr*)(hent->h_addr_list[0])));
	    lc_log_basic(LOG_NOTICE, "%s", ts_str(windex_value));
        }
        else {
	    lc_log_basic(LOG_NOTICE, "inside hent address list  is NULL");
	    err = ts_new_sprintf(&msg, _("Not able to resolve the hostname - %s" ),
                                     windex_value_esc);
            code = 1;
            bail_error(err);
...
//	    err = lc_err_generic_error;
    ...
    node_name = smprintf("%s/%s", ts_str(root),ts_str(windex_value));
    bail_null(node_name);

    lc_log_basic(LOG_NOTICE, "%s %d", node_name, type);

    // Porting to FIR - new API for IPv4 operations.
    lc_log_basic(LOG_NOTICE,"%d", lia_str_is_ipv4addr_quick(ts_str(windex_value)));
    lc_log_basic(LOG_NOTICE,"%s", ts_str(windex_value));

    node_name = smprintf("/ntp/server/address/%s", ts_str(windex_value));
    bail_null(node_name);

    err = mdc_create_binding(web_mcc, NULL, NULL,
            bt_hostname,
            ts_str(windex_value),
            node_name);
    bail_error(err);
	...
	lc_log_basic(LOG_NOTICE, "list name %s ", list_name);
        ...
        if (list_value_fields_str) {
            ...
            for (j = 0; j < num_list_fields; ++j) {
                ...
                node_name = smprintf("%s/%s/%s/%s",
                                     ts_str(root), ts_str(windex_value),
                                     ts_str(field), list_field_value_esc);
                bail_null(node_name);

		lc_log_basic(LOG_NOTICE, " node name%s", node_name);
 ...
 list_loop_clean:
        ...
        node_name = smprintf("%s/%s/%s", ts_str(root), ts_str(windex_value),
                             ts_str(field));
        bail_null(node_name);

	lc_log_basic(LOG_NOTICE, "s node name%s", node_name);
...
int
web_config_form_ntp_init(const lc_dso_info *info, void *data)
{
