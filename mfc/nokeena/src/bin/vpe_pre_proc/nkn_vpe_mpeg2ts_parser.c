...
{	
    uint32_t pid=0;
    uint8_t adaptation_field=0, adaptation_field_length=190,payload_unit_start_indicator,payload_start_byte=0;
    uint8_t continuity_count,random_access_indicator=0;
    int32_t stream_type;
    int64_t PCR_base,PCR_ext;
    static int frame_num=0,frame_type;
	...
	if(stream_type==UNKWN_PID){
		...
		if(strm_id>>4==0x0E ){
			...
			while(AU_type==0){
			    ...
			    AU_type=handle_mpeg4AVC(data,&data_pos);
