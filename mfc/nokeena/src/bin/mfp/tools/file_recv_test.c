...
int main (int32_t argc, char* argv[]) {
	...
	if (argc <= 1) {
		printf("Usage: ./<pgm_name> <ts-file-name>\n");
		return 0;
	}
...
#define NULL_PKT (0x1fff)
	...
	while (!feof(fd)) {
		...
		fread(buff, 7*188, sizeof(char), fd);
		rc = 7*188;
		tot_pkts_recvd += 1;

		if(((uint32_t)tot_pkts_recvd) % 1000 == 0)
			printf("%lu Message received from:%s  size: %d\n",
					tot_pkts_recvd, argv[1], rc);
