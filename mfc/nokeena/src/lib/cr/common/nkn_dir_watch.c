/* 
 * Filename: nkn_dir_watch_processor.c
 * Date: 23/Jun/2011
 *
 ...
 * dir_watch_proc_mgr_t whose object is an instance of PMF
 * Queuing/Processing feature.
 * 
 * API's implemented for the aforementioned functionalities
 * 1. create_dir_watch_processor_mgr - creates an instance of a pmf
 *    processor
 * 
 * Directory Watch
 * ---------------
 ...
 * handle; inputs include the directory name, the dir_watch_proc_mgr to whom
...
extern dir_watch_proc_mgr_t *g_pmgr;
 ...
 */
void
init_nkn_dir_watch_processor(void)
{
...
create_dir_watch_processor_mgr(fnp_start_session proc)
{
    dir_watch_proc_mgr_t *pmgr;
	
    pmgr = (dir_watch_proc_mgr_t*)nkn_calloc_type(1,
					    sizeof(dir_watch_proc_mgr_t), 
    ...
    TAILQ_INIT(&pmgr->dir_watch_process_queue_head);
    pthread_mutex_init(&pmgr->dir_watch_process_queue_lock, NULL);
 ...
 */
static int32_t 
start_dir_watch_event_loop(dir_watch_proc_mgr_t* pmgr, int fd, char *path)
{
    int32_t n_events, i;
	...
	if (ev->len && (ev->mask & (IN_CLOSE_WRITE | IN_MOVED_TO))) {
	    ...
	    /* add item to queue */
	    nkn_add_dir_watch_to_queue(pmgr, path, ev->name);
	}
 ...
 */
static void 
nkn_cleanup_dir_watch_process_queue_item(dir_watch_process_queue_item_t *item)
{
 ...
 */
static int32_t 
nkn_add_dir_watch_to_queue(dir_watch_proc_mgr_t* pmgr,
		     char *base_path, char *relative_path)
{
    int32_t rc = 0;
    dir_watch_process_queue_item_t *dir_watch_item;

    /* acquire the queue lock if called without a lock */
    pthread_mutex_lock(&pmgr->dir_watch_process_queue_lock);

    /* allocate for pmf item */
    dir_watch_item = (dir_watch_process_queue_item_t*) 
	calloc(1, sizeof(dir_watch_process_queue_item_t));
    if (!dir_watch_item) {
	rc = ENOMEM;
	goto unlock_queue;
    }
    ...
    /* construct the full path, the base path should not have a
       trailing slash 
    */
    snprintf(dir_watch_item->dir_watch_path, 256, "%s/%s",
	     base_path, relative_path);
	
    /* insert into queue */
    TAILQ_INSERT_TAIL(&pmgr->dir_watch_process_queue_head, dir_watch_item, entries);
	
 unlock_queue:	
    pthread_mutex_unlock(&pmgr->dir_watch_process_queue_lock);
 ...
 */
void * 
nkn_process_dir_watch(void *private_data)
{
    dir_watch_process_queue_item_t *item, *prev = NULL;
    dir_watch_proc_mgr_t *pmgr;
    int32_t rv;
	
    pmgr = (dir_watch_proc_mgr_t*)private_data;
    rv = 0;
	...
	TAILQ_FOREACH(item, &pmgr->dir_watch_process_queue_head, entries)
	    {
		/* cleanup queue item resources */
		if (prev) {
		    nkn_cleanup_dir_watch_process_queue_item(prev);
		}

		printf("PMF file name %s\n", item->dir_watch_path);
		DBG_LOG(MSG, MOD_NETWORK, "PMF file"
			   " name %s", item->dir_watch_path);
		/* remove queue element */
		TAILQ_REMOVE(&pmgr->dir_watch_process_queue_head,
			     item, entries);

		rv = pmgr->start_session(item->dir_watch_path);
		if (rv < 0) {
...
dir_watch_th(void *param)
{
    ...
    dir_watch_proc_mgr_t *pmgr = dwto->pmgr;
    int fd = dwto->fd;

    dwto->wd = inotify_add_watch(fd, dir_watch_name,
				 IN_CLOSE_WRITE | IN_MOVED_TO);	
    if (dwto->wd < 0) {
	DBG_LOG(ERROR, MOD_NETWORK, "unable to start"
