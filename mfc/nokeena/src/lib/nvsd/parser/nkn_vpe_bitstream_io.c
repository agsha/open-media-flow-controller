...
nkn_bitstream* open_bitstream(char *in, char *mode)
{
  ...
  bs = (nkn_bitstream*)nkn_malloc_type(sizeof(nkn_bitstream),
  ...
  bs->_data = bs->_start = (unsigned char*)nkn_malloc_type(file_size, 
  					mod_vpe_charbuf);
  bs->_pos = 0;
  bs->_size = file_size;
...
nkn_bitstream* create_bitstream(unsigned int size)
{
  ...
  bs->_data = bs->_start = (unsigned char*) nkn_malloc_type(size,
  					mod_vpe_charbuf);
  bs->_pos = 0;
  bs->_size = size;
...
nkn_bitstream* 
init_bitstream(void *buf, int64_t size)
{
