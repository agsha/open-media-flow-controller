...
static int32_t vpe_http_sync_strip_fqdn(const char *fqdn, char **h,
					char **u);  
static int32_t vpe_http_sync_build_head_req(vpe_http_reader_t *hr);
static int32_t vpe_http_sync_get_resp_status(vpe_http_reader_t *hr);
static int32_t vpe_http_sync_build_br_req(vpe_http_reader_t *hr, 
					  size_t start, 
					  size_t end);
static int32_t vpe_http_sync_get_version(const char *buf, 
					 int32_t buf_size,  
					 int32_t *version, 
					 int32_t *bytes_parsed);
static size_t vpe_http_sync_read_buf(void *hr, uint8_t *buf, 
				    size_t size);
static inline size_t vpe_http_sync_read_int(const char *buf, 
					     const int32_t s);
static inline int32_t vpe_http_sync_bytes_in_line(const char *buf, 
						  const int32_t size);
static inline int32_t vpe_http_sync_send_data(
			    const vpe_http_reader_t *hr); 
static inline int32_t vpe_http_sync_read_data(vpe_http_reader_t *hr);
static int32_t vpe_http_sync_read_bin_data(vpe_http_reader_t *hr, 
					   uint8_t *buf,
					   size_t size);
static inline int32_t vpe_http_sync_get_resp_len(
					 vpe_http_reader_t *hr);  
static struct sockaddr_in* vpe_http_init_sock_addr(char const* host);
static int32_t
vpe_http_timed_recv(int32_t sd, char**buf, uint32_t len);


void*
vpe_http_sync_open(char *fname, const char *mode, size_t size)
{
    vpe_http_reader_t *hr;
    struct sockaddr_in *addr = NULL;
    int32_t rv, bytes_sent, bytes_recv, pos, skip_bytes;
    hr = (vpe_http_reader_t*)\
	nkn_calloc_type(1, sizeof(vpe_http_reader_t),
    ...
    rv = vpe_http_sync_strip_fqdn(fname, &hr->host, &hr->uri);

    if (rv) {
       goto http_open_error;
    }

    addr = vpe_http_init_sock_addr(hr->host);
    if (addr == NULL) {
 	goto http_open_error;
    }
    hr->sin = addr;
    ...
    hr->req_size = vpe_http_sync_build_head_req(hr);
    if (hr->req_size <= 0) {
	goto http_open_error;
    }

    bytes_sent = vpe_http_sync_send_data(hr);
    if (bytes_sent < 0) {
	goto http_open_error;
    }

    rv = vpe_http_sync_read_data(hr);
    if (rv < 0) {
	goto http_open_error;
    }
   
    rv = vpe_http_sync_get_resp_status(hr);
    if (rv != 200) {
	    goto http_open_error;
    ...
    do {
	rv =\
	    vpe_http_sync_bytes_in_line(&hr->resp_buf[skip_bytes], 
					hr->resp_size - skip_bytes);
	if (!strncasecmp(&hr->resp_buf[skip_bytes],
			 "Content-Length:",
			 15)) {
	    hr->content_size = \
		vpe_http_sync_read_int(&hr->resp_buf[skip_bytes + 15],
				       rv - 15);
	}
...
size_t
vpe_http_sync_read(void *buf, size_t n, size_t memb, void *desc)
{
    vpe_http_reader_t *hr;

    hr = (vpe_http_reader_t *)desc;
    return vpe_http_sync_read_buf(hr, buf, n * memb);
}

size_t
vpe_http_sync_seek(void *desc, size_t skip, size_t whence)
{
    int32_t rv;
    vpe_http_reader_t *hr;
...
static size_t
vpe_http_sync_read_buf(void *desc, uint8_t *buf, size_t size)
{
    int32_t rv;
    vpe_http_reader_t *hr;
    ...
    rv = vpe_http_sync_build_br_req(hr, hr->pos, hr->pos + bytes_to_read);
    rv = vpe_http_sync_send_data(hr);
	if (rv < 0)
		return rv;
    
    rv = vpe_http_sync_read_bin_data(hr, buf, size);
    return rv;
}

size_t
vpe_http_sync_tell(void *desc)
{
    vpe_http_reader_t *hr;
...
vpe_http_sync_read_data(vpe_http_reader_t *hr)
{
	...
	while(1) {
		...
		rv = vpe_http_timed_recv(hr->sock, &tmp_buf, 
...
static int32_t
vpe_http_sync_read_bin_data(vpe_http_reader_t* hr, uint8_t* out_data,
		size_t size) {
	...
	while (1) {
		...
		rc = vpe_http_timed_recv(sockfd, (char**)(&tmp_buf), 
		...
		if (resp_hdr_flag > 0) {
			...
			vpe_http_sync_get_resp_len(hr);
			uint32_t cont_len = hr->data_size;
			uint32_t len_to_read = cont_len -
				(buf_write_pos - buf_read_pos); 
			memcpy(out_data, buf + buf_read_pos, buf_write_pos - buf_read_pos);
...
static inline int32_t
vpe_http_sync_get_resp_len(vpe_http_reader_t *hr)
{
...
static inline int32_t
vpe_http_sync_send_data(const vpe_http_reader_t *hr)
{
...
vpe_http_sync_read_int(const char *buf, const int32_t size)
{
    char *p1, *p2, z[256];
    size_t s, len, rv;
...
static inline int32_t
vpe_http_sync_bytes_in_line(const char *buf, const int32_t size)
{
    char *p;
    int32_t s;
...
static int32_t
vpe_http_sync_get_resp_status(vpe_http_reader_t *hr)
{
    ...
    vpe_http_sync_get_version(p1, p2 - p1, &ver, &pos);
    if (ver == -1) {
...
static int32_t
vpe_http_sync_get_version(const char *buf, int32_t buf_size, 
...
static int32_t
vpe_http_sync_build_head_req(vpe_http_reader_t *hr)
{
    int32_t len;
...
static int32_t
vpe_http_sync_build_br_req(vpe_http_reader_t *hr, size_t start, 
			   size_t end)
{
    int32_t len;
...
static int32_t
vpe_http_sync_strip_fqdn(const char *fqdn, char **h, char **u)
{
    char *p, *p1, *host, *uri;
    int32_t len;
...
vpe_http_init_sock_addr(char const* host) {
...
vpe_http_timed_recv(int32_t sd, char**buf, uint32_t len) {
	...
	if (rc < 0) {
		perror("vpe_http_timed_recv: select: ");
        DBG_MFPLOG ("HTTP_RECV", MSG, MOD_MFPFILE,
				"Select() failed");
		return -2;
	}
	if (FD_ISSET(sd, &read_set)) {
		...
		if (rc < 0) {
			perror("vpe_http_timed_recv: recv: ");
			DBG_MFPLOG ("HTTP_RECV", MSG, MOD_MFPFILE,
					"recv() failed");
			return -3;
		}
