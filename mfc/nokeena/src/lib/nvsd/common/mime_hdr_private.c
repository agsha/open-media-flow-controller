...
STATIC
    int add_name_value(mime_header_t * hd, dataddr_t * list_head,
		       ...
		       u_int8_t user_attr, int allow_dups)
{
    int rv = 0;
	...
	while (hdptr) {
	    ...
	    if (!allow_dups && (pheap_data->u.nv.name_len == namelen) &&
...
/*
 *******************************************************************************
 * int_mime_hdr_add_unknown_any() -- Add unknown header allowing duplicate
 *   name elements.
 *
 *   Return 0 => Success
 *******************************************************************************
 */
STATIC
int int_mime_hdr_add_unknown_any(mime_header_t * hd, const char *name, 
...
/*
 *******************************************************************************
 * atol_len_err() -- [ptr, len] atoi for long
 *******************************************************************************
 */
long atol_len_err(const char *str, int len, int *err)
{
    long sign = 1;
    long val = 0;
		   ...
		   int outbufsz, int *outbytesused /* Not including NULL */ )
{
  ...
  exit:
	...
	*outbytesused = outbuf_ix;
	...
	return unescape_str(tbuf, outbuf_ix-1, query_str, outbuf,
			...
			const char *val_end)
{
	...
	/* PR 765171
	 * Convert the value and check is there was any error
	 * If there is error in conversion do nothing and just
	 * return
	 */
	lval = atol_len_err(val, val_end - val + 1, &err);
	if (err) {
	    return rv;
	}
	...
	case CT_MAX_AGE:
	    ...
	    cdp->u.dt_cachecontrol_enum.max_age = lval;
	    break;
	case CT_MAX_STALE:
	    cdp->u.dt_cachecontrol_enum.max_stale = lval;
	    break;
	case CT_MIN_FRESH:
	    cdp->u.dt_cachecontrol_enum.min_fresh = lval;
	    break;
	case CT_S_MAXAGE:
	    cdp->u.dt_cachecontrol_enum.s_maxage = lval;
	    break;
	default:
	    {
...
/*
 * Note: EXT_DATA_ADDR() computes the NEW location where the data will reside.
 *       The actual copy of the data occurs in mime_hdr_pack_ext_data().
 *       Within the scope of this procedure, EXT_DATA_ADDR() will always
 *       point to uninitialized data.
 */
	...
	rv = int_mime_hdr_add_unknown_any(out_hdr, pname, namelen, pdata, 
					  datalen);
	if (rv) {
	    DBG("int_mime_hdr_add_unknown_any() failed rv=%d", rv);
	    retval = 7;
	    goto exit;
	}
	n++;
