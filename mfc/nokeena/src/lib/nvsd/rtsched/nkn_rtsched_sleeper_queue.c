...
GList *nkn_rttask_push_sleepers(char *obj_id, GList *in_list)
{
	...
	tmp_list = (GList *) c_rtsleeper_q_tbl_get(obj_id);
	...
	c_rtsleeper_q_tbl_insert(obj_id, ret_list);
...
*/
void
nkn_rttask_pop_sleepers(char *obj_id, GList **a_in_ptr)
{
	GList *tmp_list;
	...
	*/
	c_rtsleeper_q_tbl_delete(obj_id);
}
...
*/
int
nkn_rttask_helper_push_task_on_list(char *obj_id, void *in_ptr)
{
	GList *tmp_list = NULL;
	int ret_val = -1;
	int new_list = 0;
	...
	if (new_list == 1) {
		ret_val = c_rtsleeper_q_tbl_insert(obj_id, tmp_list);
		if(ret_val < 0) {
			return -1;
		}
	}
