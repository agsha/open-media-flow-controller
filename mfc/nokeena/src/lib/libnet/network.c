	...
	while( !LIST_EMPTY(&pnmthr->sbq_head[cur_sbq]) )
	{
		...
		if( NM_CHECK_FLAG(pnm, NMF_IN_USE) ) {
			...
			NKN_MUTEX_UNLOCKSTAT(&pnm->pthr->nm_mutex);
                        ...
                        */
                        if (cur_time.tv_sec != pnm->last_active) {
                                http_try_to_sendout_data((con_t *)pnm->private_data);
                        } else {
...
#ifdef LIB_NET
	prctl(PR_SET_NAME, "lnet-network-send", 0, 0, 0);
#else
...
static void * NM_main_loop( void * arg )
{
...
#ifdef LIB_NET
	snprintf(name, 64, "lnet-net-%d", pnmthr->num);
#else
	snprintf(name, 64, "nvsd-net-%d", pnmthr->num);
#endif
	...
	snprintf(name, 64, "net_thread%d_cpu%d_sb_queue_size", pnmthr->num, cpu);
	nkn_mon_add(name, NULL, &pnmthr->cur_sbq, sizeof(pnmthr->cur_sbq));
	snprintf(name, 64, "nt%d_mempool", pnmthr->num);

	nkn_mem_create_thread_pool((const char *)name);

	snprintf(name, 64, "monitor_%d_network_thread", pnmthr->num);
...
void NM_main( void )
{
	...
	AO_fetch_and_add(&glob_socket_cur_open_ipv6, -glob_tot_svr_sockets_ipv6);
        ...
        if (nm_hdl_send_and_receive) {
                ...
                NM_send_threads = NM_tot_threads/24;
                NM_send_threads += (NM_tot_threads%24)?1:0;
