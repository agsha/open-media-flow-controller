...
{
    unsigned int b_index = s->index;
    uint8_t result = s->buffer[b_index>>3];
#ifdef BITSTREAM_READER_LE
    result >>= b_index & 7;
    result &= 1;
#else
...
#define GET_VLC(code, name, gb, table, bits, max_depth)         \
    do {                                                        \
        ...
        unsigned int t_index;                                   \
                                                                \
        t_index = SHOW_UBITS(name, gb, bits);                   \
        code  = table[t_index][0];                              \
        n     = table[t_index][1];                              \
                                                                \
        if (max_depth > 1 && n < 0) {                           \
            ...
            nb_bits = -n;                                       \
                                                                \
            t_index = SHOW_UBITS(name, gb, nb_bits) + code;     \
            code  = table[t_index][0];                          \
            n     = table[t_index][1];                          \
            if (max_depth > 2 && n < 0) {                       \
...
#define GET_RL_VLC(level, run, name, gb, table, bits, max_depth, need_update) \
    do {                                                                \
        ...
        if (max_depth > 1 && n < 0) {                                   \
            ...
            nb_bits = -n;                                               \
                                                                        \
            t_index = SHOW_UBITS(name, gb, nb_bits) + level;            \
            level = table[t_index].level;                               \
            n     = table[t_index].len;                                 \
        }                                                               \
        run = table[t_index].run;                                       \
        SKIP_BITS(name, gb, n);                                         \
    } while (0)
