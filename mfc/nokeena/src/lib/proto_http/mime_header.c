		       ...
		       int outbuf_size)
{
	...
	DBG("Invalid parameters outbuf=%p req_bufsize=%p outbuf_size=%d "
	    "req bufsize=%d",
	    outbuf, req_bufsize, outbuf_size, req_bufsize);
	rv = 2;
    } else {
	...
	if (hd->ext_data && pshdr->ext_data_size) {
	     ...
	     * keep mime_hdr_pack_ext_data() happen even though pdest is
	     * smaller.   
			 ...
			 int extdatabufsz)
{
	...
	for (n = 0, kcnt = 0; n < MIME_MAX_HDR_DEFS; n++) {
	    if (isset(shdr.known_header_bitmap, n)) {
	    	hd->known_header_map[n] = sparse_known_header_map[kcnt++];
	    }
	}
			 ...
			 int only_end2end_hdrs, mime_header_t * hdr)
{
    ...
    id.u.attr.protocol_id = NKN_PROTO_HTTP_LIB;
    id.u.attr.id = NKN_ATTR_MIME_HDR_ID;

    serialbuf = nkn_attr_get((nkn_attr_t *)nkn_attr, id, 
			     (uint32_t *)&serialbufsz);
    if (!serialbuf) {
	DBG("Bad attribute data, nkn_attr_get(NKN_PROTO_HTTP_LIB, "
	    "NKN_ATTR_MIME_HDR_ID) failed", 0);
	rv = 3;
	goto exit;
    }
			    ...
			    int urilen, int encoding_type, nkn_objv_t *objvp)
{
	...
	// Use monatomic counter
	AO_fetch_and_add_full(&obj_ver_counter.ver.t_ver.counter, 1);
	val = obj_ver_counter.ver.fin_version;  // uuid + counter
	vallen = sizeof(obj_ver_counter.ver.fin_version);
	break;
    } /* end while */
...
#define MAX_COMMA (1)
	...
	if (!w_data) {
	    DBG("alloca() failed, requested_size=%d",
