...
#ifdef PROTO_HTTP_LIB
static AO_t glob_tot_size_from_origin;
static AO_t glob_tot_size_from_cache;
static AO_t glob_tot_size_from_sas_disk;
static AO_t glob_tot_size_from_disk;
static AO_t glob_tot_size_from_sata_disk;
static AO_t glob_tot_size_from_ssd_disk;
static AO_t glob_tot_size_from_nfs;
static AO_t glob_tot_size_from_tfm;
#else /* !PROTO_HTTP_LIB */
extern AO_t glob_tot_size_from_origin;
extern AO_t glob_tot_size_from_cache;
extern AO_t glob_tot_size_from_sas_disk;
extern AO_t glob_tot_size_from_disk;
extern AO_t glob_tot_size_from_sata_disk;
extern AO_t glob_tot_size_from_ssd_disk;
extern AO_t glob_tot_size_from_nfs;
extern AO_t glob_tot_size_from_tfm;
#endif /* !PROTO_HTTP_LIB */

#ifdef PROTO_HTTP_LIB
static
const char * get_http_content_type(const char *suffix, int len)
{
    UNUSED_ARGUMENT(suffix);
    UNUSED_ARGUMENT(len);
    return "text/html";
}
#else
...
typedef int nkn_attr_t ;
static time_t nkn_cur_ts; // always zero, suppress Age header
                         ...
                         mime_header_t *hdr)
{
    ...
    UNUSED_ARGUMENT(only_end2end_hdrs);
    UNUSED_ARGUMENT(hdr);
    return 0;
}

typedef char * pe_ilist_t;
typedef char * pe_rules_t;

static
pe_rules_t *pe_create_rule(policy_engine_config_t *pe_config, 
			   pe_ilist_t **pp_list)
{
    UNUSED_ARGUMENT(pe_config);
    UNUSED_ARGUMENT(pp_list);
    return 0;
}

static 
uint64_t pe_eval_http_send_response(pe_rules_t *p_perule, void *p_http, 
				    void *p_resp_hdr)
{
    UNUSED_ARGUMENT(p_perule);
    UNUSED_ARGUMENT(p_http);
    UNUSED_ARGUMENT(p_resp_hdr);
    return 0;
}
...
static int suppress_response_hdr(const namespace_config_t *nsc, 
...
static void http_add_via_and_date_header(http_cb_t * phttp, mime_header_t * p_response_hdr)
{
	...
	if (!modify_date) {
		...
		char * via_list   = NULL;
                ...
                */
                if (!(nsc && nsc->http_config->origin.u.http.use_client_ip)) {
